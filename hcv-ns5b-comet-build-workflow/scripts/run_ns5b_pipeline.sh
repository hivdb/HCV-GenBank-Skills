...
announce_step 9 "Build subtype study workbook from Comet" \
  "genotype workbook: $OUTPUT_DIR/NS5B_GT_AllStudies.xlsx; Comet subtype assignments: $COMET_SUBTYPE_CSV" \
...
announce_step 10 "Extract genotype-position amino-acid sequences" \
  "subtype workbook: $OUTPUT_DIR/NS5B_Subtype_AllStudies_WSeqs.xlsx; filtered FASTAs: $INCLUDED_FASTA_DIR" \
...
announce_step 11 "Build complete profile workbooks" \
  ...
  "profile workbooks: $OUTPUT_DIR/NS5B_GT_CompleteProfiles_TabsPerGT.xlsx; $OUTPUT_DIR/NS5B_Subtype_CompleteProfiles_TabsPerGT.xlsx; profile accessions: $OUTPUT_DIR/NS5B_Profile_Input_Accessions.csv"
...
"$PYTHON_BIN" "$SCRIPT_DIR/build_ns5b_completeprofiles_tabspergt.py" \
  ...
  --output-dir "$OUTPUT_DIR" \
  --profile-accessions-csv "$OUTPUT_DIR/NS5B_Profile_Input_Accessions.csv" \
...
"$PYTHON_BIN" "$SCRIPT_DIR/compare_profile_input_assignments.py" \
  --comet-csv "$OUTPUT_DIR/NS5B_Profile_Input_Accessions.csv" \
  --local-csv "$REPO_ROOT/outputs/local_alignment/NS5B_Profile_Input_Accessions.csv" \
  --summary-csv "$OUTPUT_DIR/NS5B_Profile_Input_Assignment_Comparison.csv" \
  --differences-csv "$OUTPUT_DIR/NS5B_Profile_Input_Assignment_Differences.csv"
