...
- `NS3_AllSeq_NonComet_Coverage.csv`, `NS5A_AllSeq_NonComet_Coverage.csv`, and `NS5B_AllSeq_NonComet_Coverage.csv`: supply priority non-COMET subtypes and per-gene coverage. Only rows with `FullyCover=Yes` contribute to the coverage counts.
...
It also updates `HCV_BlastHists_Apr29_with_subtype_coverage.xlsx`. In each `Original_NS3`, `Original_NS5A`, and `Original_NS5B` sheet, non-empty `RefID` rows receive `CometSubtypes` and the gene-specific `Include...` coverage count. Blank subtype values are written as `(unassigned)`.
