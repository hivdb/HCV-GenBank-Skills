...
FILTERS = {
    "NS3": {
        ...
        "2116": "source_collection_date before 2011",
        "943": "source_isolate contains Day 1",
        "2150": "source_isolate contains b",
        "600": "source_isolate does not contain failure",
        "884": "source_isolate contains Pre-TH",
        "499": "source_isolate contains HCC",
        "661": "source_isolation_source equals plasma",
        "2168": "source_isolate contains pre",
        "1356": "source_isolate does not contain IC",
        "192": "source_isolate contains day 1",
        "2138": "source_isolate contains Week 0",
    ...
    "NS5A": {
        ...
        "17": "Accession membership list: HCVData/Ref-selection/NS5_Ref_filter/NS5A/17.csv",
        "288": "source_isolate contains pre",
        "346": "source_isolate contains baseline/D0",
    },
    "NS5B": {
        "891": "source_isolate contains token Ha01 through Ha97",
        "30": "source_isolate contains day1",
...
def main() -> None:
    ...
    for gene in ("NS3", "NS5A", "NS5B"):
        ...
        for values in sheet.iter_rows(min_row=2, values_only=True):
            ...
            filter_information = FILTERS[gene].get(
                refid, "No RefID-specific filter configured in the current workflow"
            )
            result_path = (
                WORKFLOW_TEMP_ROOTS[gene]
                / "refid_metadata"
                / f"RefID_{refid}_metadata.csv"
            )
            rows.append(
                {
                    ...
                    "RefID": refid,
                    "NumPtsValue": num_pts,
                    "CurrentIncludedRefsCheck": "Present"
                    if refid in current_selection_refids
                    else "Absent",
                    "FilterInformation": filter_information,
                    "FilterResultFile": str(result_path)
                    if refid in FILTERS[gene] and result_path.is_file()
                    else "",
                    "FilterResultCreation": filter_result_creation(
                        gene, refid, filter_information
                    ),
                }
            )
    workbook.close()
    ...
    with output.open("w", encoding="utf-8", newline="") as handle:
        writer = csv.DictWriter(
            ...
            fieldnames=[
                ...
                "RefID",
                "NumPtsValue",
                "CurrentIncludedRefsCheck",
                "FilterInformation",
                "FilterResultFile",
                "FilterResultCreation",
            ],
            lineterminator="\n",
        )
        writer.writeheader()
