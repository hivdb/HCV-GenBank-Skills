...
def assignment_calls(
    ...
    dict[str, dict[str, dict[str, str]]], list[str], dict[str, list[str]]
]:
    ...
    genotype_for_subject = lambda subject: (
        match.group(1)
        if (match := re.search(r"genotype=([1-8])(?:\||$)", subject))
        else ""
    )
    genotype_labels = sorted(
        {
            label
            for header in fasta_header_records(GT_REFERENCES)
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="hcv_allseq_coverage_") as temp:
        temp_dir = Path(temp)
        assignments_by_gene, genotype_labels, subtype_labels_by_genotype = assignment_calls(
        ...
        for output in output_paths.values():
            print(output)
        calls = assignments_by_gene["NS3"]
        genotype_report = args.output_dir / "Genotype_Distances.csv"
        write_choice_report(
            genotype_report, genotype_labels, "Genotype", calls, "genotype_distances"
        )
        print(genotype_report)
        for genotype in genotype_labels:
            subtype_report = args.output_dir / f"Subtype_Distances_Genotype_{genotype}.csv"
            calls_for_genotype = {
                accession: call
                for accession, call in calls.items()
                if call.get("genotype") == genotype
            ...
            write_choice_report(
                subtype_report,
                subtype_labels_by_genotype.get(genotype, []),
                "Subtype",
                calls_for_genotype,
                "subtype_distances",
        ...
        workbook = args.output_dir / "Subtyping_Distances.xlsx"
        write_distance_workbook(
            workbook, genotype_labels, subtype_labels_by_genotype, calls
        )
        print(workbook)
