...
def read_calls(path: Path, accession_column: str, genotype_column: str, subtype_column: str) -> tuple[dict[str, dict[str, str]], int]:
...
def main() -> int:
    ...
    for gene in GENES:
        ...
        if not coverage_path.is_file() or not assignment_path.is_file():
            missing = [str(path) for path in (coverage_path, assignment_path) if not path.is_file()]
            raise SystemExit(f"Missing required input file(s): {', '.join(missing)}")

        coverage, coverage_rows = read_calls(coverage_path, "Accession", "ClosestGenotype", "ClosestSubtype")
        assignments, assignment_rows = read_calls(assignment_path, "accession", "genotype", "subtype")
        ...
        for key in shared:
            ...
            genotype_differs = normalized_call(coverage_call["Genotype"]) != normalized_call(assignment_call["Genotype"])
            subtype_differs = normalized_call(coverage_call["Subtype"]) != normalized_call(assignment_call["Subtype"])
            ...
            differences.append({
                ...
                "DifferenceType": "BOTH" if genotype_differs and subtype_differs else "GENOTYPE" if genotype_differs else "SUBTYPE",
            })

        write_csv(
            args.output_dir / f"{gene}_genotype_subtype_differences.csv",
            ["Accession", "FromDbAccession", "FromFastaAccession", "FromDbGenotype", "FromFastaGenotype", "FromDbSubtype", "FromFastaSubtype", "DifferenceType"],
            differences,
        )
        write_csv(
            args.output_dir / f"{gene}_only_in_from_db.csv",
            ["Accession", "FromDbAccession", "FromDbGenotype", "FromDbSubtype"],
            [{"Accession": key, "FromDbAccession": coverage[key]["Accession"], "FromDbGenotype": coverage[key]["Genotype"], "FromDbSubtype": coverage[key]["Subtype"]} for key in only_in_coverage],
        )
        write_csv(
            args.output_dir / f"{gene}_only_in_from_fasta.csv",
            ["Accession", "FromFastaAccession", "FromFastaGenotype", "FromFastaSubtype"],
            [{"Accession": key, "FromFastaAccession": assignments[key]["Accession"], "FromFastaGenotype": assignments[key]["Genotype"], "FromFastaSubtype": assignments[key]["Subtype"]} for key in only_in_assignments],
        )
        summary_rows.append({
            "Gene": gene,
    ...
    summary_fields = [
        "Gene", "FromDbInputRows", "FromDbUniqueAccessions", "FromFastaInputRows", "FromFastaUniqueAccessions",
        "SharedAccessions", "OnlyInFromDb", "OnlyInFromFasta", "GenotypeDifferences", "SubtypeDifferences",
        "AnyGenotypeOrSubtypeDifference",
    ]
    write_csv(args.output_dir / "accession_count_summary.csv", summary_fields, summary_rows)
