...
usage() {
  cat <<'EOF'
Usage:
  EXCEL_FILE=/path/to/HCV_BlastHits.xlsx SHEET_NAME=NS5B_PtGT0_Check FASTA_POOL=/path/to/FASTA [GENBANK_DIR=/path/to/genbank_seq_files] hcv-ns5b-build-workflow/scripts/run_ns5b_pipeline.sh
...
announce_step 8 "Build subtype study workbook" \
  "genotype workbook: $OUTPUT_DIR/NS5B_GT_AllStudies.xlsx; filtered FASTAs: $INCLUDED_FASTA_DIR" \
...
announce_step 10 "Build complete profile workbooks" \
  ...
  "profile workbooks: $OUTPUT_DIR/NS5B_GT_CompleteProfiles_TabsPerGT.xlsx; $OUTPUT_DIR/NS5B_Subtype_CompleteProfiles_TabsPerGT.xlsx; profile accessions: $OUTPUT_DIR/NS5B_Profile_Accessions.csv"
...
"$PYTHON_BIN" "$SCRIPT_DIR/export_ns5b_consensus_fasta.py" \
  ...
  --output-dir "$OUTPUT_DIR" \
  > /dev/null
"$PYTHON_BIN" "$SCRIPT_DIR/build_ns5b_subtype_consensus_reference_distance.py" --subtype-profile-workbook "$OUTPUT_DIR/NS5B_Subtype_CompleteProfiles_TabsPerGT.xlsx" --subtype-json "$SUBTYPE_JSON" --output-xlsx "$OUTPUT_DIR/NS5B_Subtype_Consensus_Reference_AA_Distance_Pos150_321.xlsx" --start 150 --end 321
...
for sequence_type in aa na; do
  ...
  "$PYTHON_BIN" "$SCRIPT_DIR/build_ns5b_${sequence_type}_distance_matrices.py" \
    --input-workbook "$AA_TMP_WORKBOOK" \
    --profile-accessions-csv "$OUTPUT_DIR/NS5B_Profile_Accessions.csv" \
    --sequence-type "$sequence_type" \
    --positions 150,159,206,282,316,320,321 \
    --gt-output-xlsx "$OUTPUT_DIR/NS5B_GT_${upper_type}_Distance_RAS.xlsx" \
    --subtype-output-xlsx "$OUTPUT_DIR/NS5B_Subtype_${upper_type}_Distance_RAS.xlsx" \
    --min-subtype-sequences 10
  "$PYTHON_BIN" "$SCRIPT_DIR/build_ns5b_${sequence_type}_distance_matrices.py" \
    --input-workbook "$AA_TMP_WORKBOOK" \
    --profile-accessions-csv "$OUTPUT_DIR/NS5B_Profile_Accessions.csv" \
    --sequence-type "$sequence_type" \
    --positions "$(seq 150 321 | paste -sd, -)" \
    --gt-output-xlsx "$OUTPUT_DIR/NS5B_GT_${upper_type}_Distance_Pos150_321.xlsx" \
    --subtype-output-xlsx "$OUTPUT_DIR/NS5B_Subtype_${upper_type}_Distance_Pos150_321.xlsx" \
    --min-subtype-sequences 10
done
