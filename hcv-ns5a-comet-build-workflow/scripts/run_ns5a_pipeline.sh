...
announce_step 9 "Build subtype study workbook from Comet" \
  "genotype workbook: $OUTPUT_DIR/NS5A_GT_AllStudies.xlsx; Comet subtype assignments: $COMET_SUBTYPE_CSV" \
...
announce_step 10 "Extract genotype-position amino-acid sequences" \
  "subtype workbook: $OUTPUT_DIR/NS5A_Subtype_AllStudies_WSeqs.xlsx; filtered FASTAs: $INCLUDED_FASTA_DIR" \
...
"$PYTHON_BIN" "$SCRIPT_DIR/compare_profile_input_assignments.py" \
  --comet-csv "$OUTPUT_DIR/NS5A_Profile_Input_Accessions.csv" \
  --local-csv "$REPO_ROOT/outputs/local_alignment/NS5A_Profile_Input_Accessions.csv" \
  --summary-csv "$OUTPUT_DIR/NS5A_Profile_Input_Assignment_Comparison.csv" \
  --differences-csv "$OUTPUT_DIR/NS5A_Profile_Input_Assignment_Differences.csv"
