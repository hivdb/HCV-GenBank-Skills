...
REFERENCE_AA_LENGTH = 591


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Export NS5B consensus amino-acid FASTA records from GT and subtype complete-profile workbooks."
    )
...
def script_temp_dir() -> Path:
    path = Path("outputs/temp") / "hcv-ns5b-build-workflow" / Path(__file__).stem
...
def main() -> int:
    ...
    gt_fasta = Path(args.gt_output_fasta).expanduser() if args.gt_output_fasta else output_dir / "NS5B_GT_Consensus.fasta"
    subtype_fasta = (
        Path(args.subtype_output_fasta).expanduser()
        if args.subtype_output_fasta
        else output_dir / "NS5B_Subtype_Consensus.fasta"
    )
