...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--input-workbook", required=True, help="Profile input alignment-QC workbook."
    )
...
def main() -> int:
    ...
    if missing:
        raise RuntimeError(
            f"Missing columns in {input_path}: {', '.join(sorted(missing))}"
        )

    by_genotype: dict[str, dict[str, list[float]]] = defaultdict(
    ...
    for genotype in sorted(
        by_genotype, key=lambda value: int(value) if value.isdigit() else 999
    ):
    ...
    distribution = sorted(
        output_rows, key=lambda row: (-int(row["n"]), str(row["genotype"]))
    )
    distribution_text = (
        ", ".join(
