...
def aligned_strings(reference, query, coordinates):
    ...
    for n in range(len(coordinates[0]) - 1):
        rs, re, qs, qe = (
            int(coordinates[0][n]),
            int(coordinates[0][n + 1]),
            int(coordinates[1][n]),
            int(coordinates[1][n + 1]),
        )
        if re - rs and qe - qs:
            out[0].append(reference[rs:re])
            out[1].append(query[qs:qe])
        elif re - rs:
            out[0].append(reference[rs:re])
            out[1].append("-" * (re - rs))
        else:
...
def main():
    ...
    for r in json.loads(Path(a.subtype_json).read_text()):
        ...
        if m and m.group(1).lower() not in refs:
            nt = str(r.get("sequence", "")).upper()
            refs[m.group(1).lower()] = (
                str(r.get("accession", "")),
                "".join(
                    CODONS.get(nt[i : i + 3], "X")
                    for i in range(0, min(len(nt), 1893) - 2, 3)
                ),
            )
    wb = load_workbook(a.subtype_profile_workbook, read_only=True, data_only=True)
    out = Workbook()
    wsout = out.active
    wsout.title = "subtype_reference_distance"
    wsout.append(
        [
            "Genotype",
            "Subtype",
    ...
    for sheet in wb.sheetnames:
        ...
        for row in ws.iter_rows(min_row=2, values_only=True):
            st = str(row[ix["Subtype"]] or "").strip().lower()
            pos = row[ix[[x for x in h if x.endswith("Position")][0]]]
            aa = str(row[ix["AminoAcid"]] or "").upper()
            pct = row[ix["PctWithAA"]]
            if st and pos and pct is not None:
                calls = seqs.setdefault(st, {})
                current = calls.get(int(pos))
                if (
                    current is None
                    or float(pct) > current[0]
                    or (float(pct) == current[0] and aa < current[1])
                ):
                    calls[int(pos)] = (float(pct), aa)
        for st, calls in sorted(seqs.items()):
            ref = refs.get(st)
            key = f"{gt}{st[len(gt) :]}" if st.startswith(gt) else st
            if ref is None:
                wsout.append([gt, st, "", "", "", "", "", "reference_not_found"])
                continue
            acc, refaa = ref
            consensus = "".join(
                calls.get(pos, (0, "X"))[1] for pos in range(1, len(refaa) + 1)
            )
            ...
            alignment = aligner.align(refaa, consensus)[0]
            aligned_ref, aligned_consensus = aligned_strings(
                refaa, consensus, alignment.coordinates
            )
            refpos = 0
            pairs = []
            for refaa_char, consensus_char in zip(aligned_ref, aligned_consensus):
                ...
                if (
                    refaa_char != "-"
                    and refpos in positions
                    and refaa_char in VALID
                    and consensus_char in VALID
                ):
                    pairs.append((refpos, refaa_char, consensus_char))
            mismatches = [
            ...
            wsout.append(
                [
                    ...
                    diff,
                    ";".join(mismatches),
                    diff / len(pairs) if pairs else "",
                    "ok" if pairs else "no_comparable_positions",
                ]
            )
    wb.close()
    meta = out.create_sheet("metadata")
    meta.append(["aa_positions", ",".join(map(str, positions))])
    meta.append(
        [
            "alignment",
    ...
    meta.append(
        ["distance_definition", "AA differences / comparable aligned NS3 RAS positions"]
    )
