---
name: hcv-ns5a-all-ras-comet-build-workflow
...
```bash
python hcv-workflow/hcv-ns5a-all-ras-comet-build-workflow/scripts/run_ns5a_pipeline.py --list-steps
python hcv-workflow/hcv-ns5a-all-ras-comet-build-workflow/scripts/run_ns5a_pipeline.py
python hcv-workflow/hcv-ns5a-all-ras-comet-build-workflow/scripts/run_ns5a_pipeline.py --step discover-refid-fastas
```
...
The variant inherits settings from `[common]` and `[ns5a_comet]`. Add `[ns5a_all_ras]` to `pipeline.local.toml` to set its isolated `output_dir` and, when needed, a different `sheet_name`.
Each step writes its files under `outputs/comet-NS5A-all-ras/<order>_<step-name>/` (for example, `09_build-genotype-workbook/`). The runner prints each step as a Markdown heading with its order, name, and short explanation, followed by input, excluded, and final-included accession counts.
...
- `HCVData/nonComet-Full-genome/NS5A_AllSeq_NonComet_Coverage.csv` for mandatory non-COMET genotype 7/8 overrides and additions
...
Complete-profile construction requires a callable standard amino acid at every NS5A RAS position: 24, 26, 28, 29, 30, 31, 32, 38, 58, 62, 92, and 93. Missing, `X`, stop (`*`), or non-standard calls at any one of these positions exclude the accession from the profile and downstream combined RAS reports.
...
The workflow writes NS5A outputs under `outputs/comet-NS5A-all-ras/`, in numbered step folders, including:
...
- Keep every generated artifact in its numbered step folder under `outputs/comet-NS5A-all-ras/`.
