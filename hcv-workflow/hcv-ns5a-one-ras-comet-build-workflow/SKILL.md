---
name: hcv-ns5a-one-ras-comet-build-workflow
...
```bash
python hcv-workflow/hcv-ns5a-one-ras-comet-build-workflow/scripts/run_ns5a_pipeline.py --list-steps
python hcv-workflow/hcv-ns5a-one-ras-comet-build-workflow/scripts/run_ns5a_pipeline.py
python hcv-workflow/hcv-ns5a-one-ras-comet-build-workflow/scripts/run_ns5a_pipeline.py --step discover-refid-fastas
```
...
The variant inherits settings from `[common]` and `[ns5a_comet]`. Add `[ns5a_one_ras]` to `pipeline.local.toml` to set its isolated `output_dir` and, when needed, a different `sheet_name`.
Each step writes its files under `outputs/comet-NS5A-one-ras/<order>_<step-name>/` (for example, `09_build-genotype-workbook/`). The runner prints each step as a Markdown heading with its order, name, and short explanation, followed by input, excluded, and final-included accession counts.
...
- Keep every generated artifact in its numbered step folder under `outputs/comet-NS5A-one-ras/`.
