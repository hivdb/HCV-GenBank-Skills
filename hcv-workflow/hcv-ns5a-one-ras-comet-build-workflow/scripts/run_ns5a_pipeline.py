...
SKILL_NAME = "hcv-ns5a-one-ras-comet-build-workflow"
...
STEP_NAMES = (
    ...
    "build-complete-profiles", "merge-subtype-complete-profiles", "export-noncomet-priority-accessions",
...
def load_config(config_path: Path) -> dict[str, str]:
    ...
    if config_path.is_file():
        ...
        for section_name in ("common", "ns5a_comet", "ns5a_one_ras"):
...
def format_genotype_distribution(distribution: dict[str, int]) -> str:
    total = sum(distribution.values())
    if not total:
        return "none"
    entries = []
    for genotype, count in sorted(distribution.items(), key=lambda item: (-item[1], item[0])):
        percentage = 100.0 * count / total
        percentage_text = f"{percentage:.2f}%" if percentage < 1 else f"{percentage:.0f}%"
        entries.append(f"GT{genotype} ({percentage_text}, {count})")
    return ", ".join(entries)
...
class Pipeline:
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        self.output_dir = path_value(value("output_dir", "OUTPUT_DIR", "outputs/comet-NS5A-one-ras"))
        ...
        self.comet_subtype_assignments_csv = self.temp_root / "comet_ns5a_subtype_assignments.csv"
        self.priority_assignments_csv = self.step_dir("select-noncomet-priority-assignments") / "NS5A_NonComet_Priority_Assignments.csv"
    ...
    def print_gt7_gt8_subtype_counts(self) -> None:
        ...
        if self.current_step_order >= STEP_ORDER["build-complete-profiles"] and self.profile_accessions_csv.is_file():
            ...
            extraction_workbook = self.step_dir("extract-profile-aa") / "NS5A_Profile_Input_Source.xlsx"
            ...
            with self.comet_subtype_assignments_csv.open(encoding="utf-8-sig", newline="") as handle:
                for row in csv.DictReader(handle):
                    ...
                    if accession:
                        comet_assignments[accession.split(".", 1)[0]] = ((row.get("genotype") or "").strip().removeprefix("GT"), (row.get("subtype") or "").strip().lower())
            ...
            if self.priority_assignments_csv.is_file():
                with self.priority_assignments_csv.open(encoding="utf-8-sig", newline="") as handle:
                    for row in csv.DictReader(handle):
                        accession = (row.get("Accession") or "").strip()
                        if accession:
                            priority_assignments[accession.split(".", 1)[0]] = ((row.get("ClosestGenotype") or "").strip().removeprefix("GT"), (row.get("ClosestSubtype") or "").strip().lower())
    ...
    def steps(self) -> list[Step]:
        ...
        explicit_subtype_ras = self.step_dir("build-subtype-ras-profile") / "NS5A_Subtype_RAS_Profiles_Explicit_AA.xlsx"
        ...
        def discover() -> None:
            output = self.run("find_refid_fastas.py", "--excel-file", self.excel_file, "--sheet", self.sheet_name, "--fasta-dir", self.fasta_pool, "--output-dir", self.discovery_tmp, stdout_path=self.discovery_tmp / "discovery_ns5a.json", stream_output=True)
            del output
        ...
        def report_profile_counts() -> None:
            ...
            print(f"Complete-profile genotype distribution: {format_genotype_distribution(summary.get('included_genotype_distribution', {}))}")
        ...
        return [
            ...
            Step("merge-subtype-complete-profiles", "merge subtype complete-profile worksheets into one table", lambda: self.run("merge_ns5a_subtype_completeprofiles.py", "--input-workbook", subtype_profile, "--output-workbook", self.step_dir("merge-subtype-complete-profiles") / "NS5A_Subtype_CompleteProfiles_Merged.xlsx", stdout_path=summary("merge-subtype-complete-profiles"))),
...
def main() -> int:
    ...
    for name in requested:
        ...
        pipeline.announce(step.order, step.name, step.description)
        input_fasta_dir = pipeline.staged_fasta_dir if step.order <= STEP_ORDER["stage-refid-fastas"] else (pipeline.included_fasta_dir if step.order <= STEP_ORDER["filter-refid-fastas"] else pipeline.filtered_fasta_dir)
