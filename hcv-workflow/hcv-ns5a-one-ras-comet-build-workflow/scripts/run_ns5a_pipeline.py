...
STEP_NAMES = (
    ...
    "build-genotype-aa-distance",
    "build-subtype-aa-distance",
...
# Keep the established integer stage numbers unchanged. The accession-level
# wide table is an auxiliary stage immediately following step 17.
STEP_ORDER = {
...
STEP_LABEL = {
    name: f"{int(order):02d}" for name, order in STEP_ORDER.items()
}
...
def format_genotype_distribution(distribution: dict[str, int]) -> str:
    ...
    for genotype, count in sorted(
        ...
        percentage_text = (
            f"{percentage:.2f}%" if percentage < 1 else f"{percentage:.0f}%"
        )
...
class Pipeline:
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        self.output_dir = path_value(
            value("output_dir", "OUTPUT_DIR", "outputs/comet-NS5A-one-ras")
        )
    ...
    def print_accession_counts(self, input_accessions: set[str]) -> None:
        ...
        if (
            ...
            and self.profile_accessions_csv.is_file()
        ):
            with self.profile_accessions_csv.open(
    ...
    def print_gt7_gt8_subtype_counts(self) -> None:
        ...
        if (
            ...
            extraction_workbook = (
                self.step_dir("extract-profile-aa") / "NS5A_Profile_Input_Source.xlsx"
            )
    ...
    def steps(self) -> list[Step]:
        ...
        gt_workbook = (
            self.step_dir("build-genotype-workbook") / "NS5A_GT_AllStudies.xlsx"
        )
        subtype_workbook = (
            self.step_dir("build-subtype-workbook")
            / "NS5A_Subtype_AllStudies_WSeqs.xlsx"
        ...
        gt_consensus = (
            self.step_dir("export-consensus-fastas") / "NS5A_GT_Consensus.fasta"
        )
        subtype_consensus = (
            self.step_dir("export-consensus-fastas") / "NS5A_Subtype_Consensus.fasta"
        )
        gt_ras = (
            self.step_dir("build-genotype-ras-profile") / "NS5A_GT_RAS_Profiles.xlsx"
        )
        subtype_ras = (
        ...
        explicit_subtype_ras = (
            self.step_dir("build-subtype-ras-profile")
            / "NS5A_Subtype_RAS_Profiles_Explicit_AA.xlsx"
        )
        combined_ras = (
            self.step_dir("build-combined-ras-reports")
            / "NS5A_Combined_RAS_Profiles.xlsx"
        )
        annotated_combined_ras = (
            self.step_dir("add-nonconsensus-row")
            / "NS5A_Combined_RAS_Profiles_Annotated.xlsx"
        )
        ...
        def prepare() -> None:
            self.run(
                "prepare_ns5a_pipeline_workdirs.py", "--clean-dir", self.output_dir
            )
        ...
        def discover() -> None:
            output = self.run(
                ...
                "--output-dir",
                self.discovery_tmp,
                stdout_path=self.discovery_tmp / "discovery_ns5a.json",
                stream_output=True,
            )
            del output
            matches = sorted(
        ...
        def prepare_comet() -> None:
            ...
            self.run(
                "prepare_comet_ns5a_assignments.py",
                "--comet-csv",
                self.comet_csv,
                "--fasta-dir",
                self.included_fasta_dir,
                ...
                self.temp_root / "comet_ns5a_not_found_or_unassigned.csv",
                "--not-found-fasta-output",
                self.temp_root / "comet_ns5a_not_found_or_unassigned.fasta",
                "--unassigned-count-output",
                self.step_dir("prepare-comet-assignments")
                / "NS5A_Comet_Unassigned_Accession_Count.txt",
                "--remove-unassigned",
            )
        ...
        def extract_aa() -> None:
            ...
            self.run(
                "build_ns5a_subtype_with_gt_aa.py",
                "--subtype-workbook",
                subtype_workbook,
                "--fasta-dir",
                self.fasta_pool,
                ...
                step_dir,
                "--output-workbook",
                step_dir / "NS5A_Profile_Input_Source.xlsx",
        ...
        def report_profile_counts() -> None:
            ...
            print(
                f"Complete-profile genotype distribution: {format_genotype_distribution(summary.get('included_genotype_distribution', {}))}"
            )
            ...
            for sequence_type in ("aa", "na"):
                ...
                for positions, suffix in (
                    ...
                    self.run(
                        ...
                        positions,
                        "--gt-output-xlsx",
                        step_dir / f"NS5A_GT_{upper}_Distance_{suffix}.xlsx",
                        "--subtype-output-xlsx",
        ...
        return [
            Step(
                "prepare-workdirs",
                "recreate run directories and clear stale source-feature files",
                prepare,
            ),
            Step(
            ...
            Step(
                ...
                Step(
                    ...
                    lambda: (
                        self.run(
                            ...
                            self.step_dir("merge-subtype-complete-profiles") / "NS5A_Subtype_CompleteProfiles_Pos24_93.xlsx",
                            "--range-start", "24", "--range-end", "93",
                ...
                Step(
                    "export-accession-aa-wide-table",
                    "export accession amino-acid calls at positions 24 through 93",
                    lambda: self.run(
                        ...
                        / "NS5A_Profile_Accession_AA_Calls_Wide_Pos24_93.csv",
                        "--copy-output-csv",
                        REPO_ROOT
                        / "covariant_analysis/NS5A_Profile_Accession_AA_Calls_Wide_Pos24_93.csv",
            ...
            Step(
                ...
                lambda: (
                    self.run(
                        "build_ns5a_subtype_ras_profiles.py",
                        "--subtype-profile-workbook",
                        subtype_profile,
            ...
            Step(
                ...
                lambda: (
                    self.run(
                        "build_ns5a_combined_ras_profiles.py",
                        "--gt-ras-profile-workbook",
                        gt_ras,
                    ...
                    self.run(
                        ...
                        "--output-xlsx",
                        self.step_dir("build-combined-ras-reports")
                        / "NS5A_Subtype_RAS_Coverage_Report.xlsx",
                    ),
                    self.run(
            ...
            Step(
                ...
                lambda: self.run(
                    "build_ns5a_subtype_ras_consensus_difference_summary.py",
                    "--combined-profile-workbook",
                    combined_ras,
                    "--profile-input-workbook",
                    self.aa_workbook,
                    ...
                    / "NS5A_Subtype_RAS_Consensus_Difference_Summary.xlsx",
                    "--output-png",
                    self.step_dir("summarize-subtype-ras-differences")
                    / "NS5A_Subtype_RAS_Consensus_Difference_Trend.png",
                    stdout_path=summary("summarize-subtype-ras-differences"),
                ),
            ),
            Step(
                "build-genotype-aa-distance",
                "build genotype amino-acid distance matrix",
                lambda: self.run(
                    "build_ns5a_gt_aa_distance_matrix.py",
                    "--input-fasta",
                    gt_consensus,
                    "--aligned-fasta",
                    self.step_dir("build-genotype-aa-distance")
                    / "NS5A_GT_Consensus_aligned.fasta",
                    "--output-xlsx",
                    self.step_dir("build-genotype-aa-distance")
                    / "NS5A_GT_AA_Distance_Pos24_93.xlsx",
                    "--details-xlsx",
                    summary(
                        "build-genotype-aa-distance",
                        "NS5A_GT_AA_Distance_Pos24_93_details.xlsx",
                    ),
                    "--start",
                    "24",
                    "--end",
                    "93",
                    stdout_path=summary(
                        "build-genotype-aa-distance", "last_run_summary.txt"
                    ),
                ),
            ),
            Step(
                "build-subtype-aa-distance",
                "build subtype amino-acid distance matrices",
                lambda: self.run(
                    "build_ns5a_subtype_aa_distance_matrices.py",
                    "--input-fasta",
                    subtype_consensus,
                    "--subtype-profile-workbook",
                    subtype_profile,
                    ...
                    "10",
                    "--output-xlsx",
                    self.step_dir("build-subtype-aa-distance")
                    / "NS5A_Subtype_AA_Distance_Pos24_93.xlsx",
                    "--temp-dir",
                    self.step_dir("build-subtype-aa-distance"),
                    "--start",
                    "24",
                    ...
                    "93",
                    stdout_path=summary(
                        "build-subtype-aa-distance", "last_run_summary.txt"
                    ),
                ),
            ),
            Step(
            ...
            Step(
                ...
                lambda: self.run(
                    "build_ns5a_ras_entropy.py",
                    "--gt-profile-workbook",
                    gt_profile,
                    "--subtype-profile-workbook",
                    subtype_profile,
                    "--gt-output-xlsx",
                    self.step_dir("build-ras-entropy") / "NS5A_GT_RAS_Entropy.xlsx",
                    "--subtype-output-xlsx",
                    self.step_dir("build-ras-entropy")
                    / "NS5A_Subtype_RAS_Entropy.xlsx",
            ...
            Step(
                ...
                lambda: self.run(
                    "analyze_ns5a_genotype_subtype_aa_predictability.py",
                    "--subtype-profile-workbook",
                    merged_subtype_profile,
                    "--min-subtype-sequences",
                    "1",
                    ...
                    / "NS5A_Genotype_Subtype_AA_Predictability.xlsx",
                    stdout_path=summary(
                        "analyze-genotype-subtype-aa-predictability",
            ...
            Step(
                "publish-ictv-report",
                "publish the NS5A ICTV comparison report",
                lambda: self.run(
            ...
            Step(
                ...
                lambda: self.run(
                    ...
                    / "NS5A_GT7_GT8_Step_Sequence_Audit.csv",
                    "--accessions-csv",
                    self.step_dir("audit-gt7-gt8-sequences")
                    / "NS5A_GT7_GT8_Step_Sequence_Audit_Accessions.csv",
                    "--summary-xlsx",
                    self.step_dir("audit-gt7-gt8-sequences")
                    / "NS5A_GT7_GT8_Step_Sequence_Audit_Summary.xlsx",
                    "--summary-markdown",
                    self.step_dir("audit-gt7-gt8-sequences")
                    / "NS5A_GT7_GT8_Step_Sequence_Audit_Summary.md",
                ),
            ),
            Step(
                "compare-gt7-gt8-local-assignments",
                "compare GT7 and GT8 workflow subtype calls with local NS5A assignments",
                lambda: self.run(
                    "compare_ns5a_gt7_gt8_local_assignments.py",
                    "--gene",
                    "NS5A",
                    "--subtype-workbook",
                    subtype_workbook,
                    ...
                    / "NS5A_GT7_GT8_Local_Assignment_Comparison.xlsx",
                    "--output-csv",
                    self.step_dir("compare-gt7-gt8-local-assignments")
                    / "NS5A_GT7_GT8_Local_Assignment_Comparison.csv",
                    stdout_path=summary("compare-gt7-gt8-local-assignments"),
                ),
            ),
            Step(
                "collect-report",
                "copy final NS5A report artifacts into the report folder",
                lambda: self.run(
