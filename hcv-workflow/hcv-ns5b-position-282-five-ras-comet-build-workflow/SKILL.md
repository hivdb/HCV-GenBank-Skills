---
name: hcv-ns5b-position-282-five-ras-comet-build-workflow
...
# HCV NS5B Position-282 Plus Five-RAS COMET Build Workflow
...
```bash
python hcv-workflow/hcv-ns5b-position-282-five-ras-comet-build-workflow/scripts/run_ns5b_pipeline.py --list-steps
python hcv-workflow/hcv-ns5b-position-282-five-ras-comet-build-workflow/scripts/run_ns5b_pipeline.py
python hcv-workflow/hcv-ns5b-position-282-five-ras-comet-build-workflow/scripts/run_ns5b_pipeline.py --step discover-refid-fastas
```
...
The variant inherits settings from `[common]` and `[ns5b_comet]`. Add `[ns5b_position_282_five_ras]` to `pipeline.local.toml` to set its isolated `output_dir` and, when needed, a different `sheet_name`.
Each stage writes its outputs under a numbered directory in `outputs/comet-NS5B-position-282-five-ras/`.
...
The workflow writes NS5B outputs under `outputs/comet-NS5B-position-282-five-ras/`, including:

- `NS5B_GT_AllStudies.xlsx`
- `NS5B_matched_fasta_files.txt`
- discovery `filtered_rows.xlsx` under `outputs/comet-NS5B-position-282-five-ras/temp/.../find_refid_fastas/...`
...
- Keep temporary outputs under `outputs/comet-NS5B-position-282-five-ras/temp/` so they do not mix with other skills.
