...
def profiles(path: Path) -> dict[tuple[str, str, int, str], str]:
    ...
    for row in ws.iter_rows(min_row=2, values_only=True):
        label = str(row[0] or "")
        m = re.match(r"GT(\d+)_(\S+) \(", label)
        if not m:
            continue
        for column, value in enumerate(row[1:], 1):
            if column >= len(header) or not str(header[column] or "").startswith("P"):
                continue
            pos = int(str(header[column])[1:])
            for aa, pct in VARIANT.findall(str(value or "")):
                result[(m.group(1), m.group(2).lower(), pos, aa)] = pct
    wb.close()
    return result
...
def main() -> None:
    ...
    parser.add_argument(
        "--ns5a-output-dir", type=Path, default=Path("outputs/comet-NS5A-all-ras")
    )
    parser.add_argument(
    ...
    for gene, token in gene_tokens:
        ...
        ws = wb.active
        col = next(
            (cell.column for cell in ws[2] if cell.value == "Mutations"),
            ws.max_column + 1,
        )
        for row in range(1, ws.max_row + 1):
            if ws.cell(row, 1).value == "Gene":
                ws.cell(row, col).value = "Mutations"
                ws.cell(row, col).font = Font(bold=True)
            if ws.cell(row, 4).value != "Consensus":
                continue
            refrow = row - 1
            gt = str(ws.cell(row, 2).value or "").removeprefix("GT")
            ...
            for c in range(5, col):
                ...
                if not cons:
                    continue
                ref = ws.cell(refrow, c).value
                pos = ws.cell(2, c).value
            ...
            if parts:
                ...
                readme.setdefault(gene, {}).setdefault(
                    f"GT{gt}_{subtype}", set()
                ).update(plain)
