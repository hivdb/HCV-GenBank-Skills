...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Multiple-align NS5A genotype amino-acid consensus FASTA, slice aligned "
            "positions 24-93, and write pairwise genotype amino-acid distances."
        )
    )
    parser.add_argument("--input-fasta", default="outputs/NS5A_GT_Consensus.fasta")
    parser.add_argument(
        "--aligned-fasta", default="outputs/NS5A_GT_Consensus_aligned.fasta"
    )
    parser.add_argument(
        "--output-xlsx", default="outputs/NS5A_GT_AA_Distance_Pos24_93.xlsx"
    )
    parser.add_argument(
        "--details-xlsx",
        default="outputs/comet-NS5A-all-ras/temp/ns5a_gt_aa_distance_matrix/NS5A_GT_AA_Distance_Pos24_93_details.xlsx",
        help="Workbook for supporting sheets that are not kept in the main output",
    )
    parser.add_argument(
        "--start", type=int, default=24, help="1-based aligned start position"
    )
    parser.add_argument(
        "--end", type=int, default=93, help="1-based aligned end position, inclusive"
    )
