...
def has_callable_ras_positions(start: int, aa_sequence: str) -> bool:
    """Return whether every NS5A RAS position has a callable standard amino-acid call."""
    sequence = aa_sequence.upper()
    return all(0 <= position - start < len(sequence) and sequence[position - start] in CALLABLE_AAS for position in NS5A_RAS_POSITIONS)
...
def main() -> int:
    ...
    summary = {
        ...
        **counts,
        ...
        "profile_input_rule": "Every retained accession has a callable amino-acid at every NS5A RAS position.",
