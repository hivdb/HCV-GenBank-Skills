...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build GT-level NS5A resistance-position AA profile summary in Excel and PNG."
    )
...
def script_temp_dir() -> Path:
    path = Path("outputs/temp") / "hcv-ns5a-build-workflow" / Path(__file__).stem
...
def make_job_dir(base_output_dir: Path, workbook_path: Path) -> Path:
    label = sanitize_label(f"{workbook_path.stem}_ns5a_gt_resistance_profile")
...
def main() -> int:
    ...
    # Historical PNG output kept for reference only.
    # png_path = job_dir / "NS5A_GT_Resistance_Profile_Summary.png"
