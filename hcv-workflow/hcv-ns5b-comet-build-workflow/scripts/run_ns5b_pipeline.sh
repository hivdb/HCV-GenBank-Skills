...
usage() {
  cat <<'EOF'
Usage:
  EXCEL_FILE=/path/to/HCV_BlastHits.xlsx SHEET_NAME=NS5B_PtGT0_Check FASTA_POOL=/path/to/FASTA [GENBANK_DIR=/path/to/genbank_seq_files] hcv-workflow/hcv-ns5b-comet-build-workflow/scripts/run_ns5b_pipeline.sh
...
COMET_SUBTYPING_CSV="${COMET_SUBTYPING_CSV:-$REPO_ROOT/HCVData/Comet Subtyping/NS5B.csv}"
NONCOMET_SUBTYPE_WORKBOOK="${NONCOMET_SUBTYPE_WORKBOOK:-$REPO_ROOT/outputs/local_alignment/NS5B_Subtype_AllStudies_WSeqs.xlsx}"
...
COMET_NOT_FOUND_FASTA="$TEMP_ROOT/comet_ns5b_not_found_or_unassigned.fasta"
mkdir -p "$TEMP_ROOT"
...
"$PYTHON_BIN" "$SCRIPT_DIR/build_ns5b_comet_gt_allstudies.py" \
  --fasta-dir "$INCLUDED_FASTA_DIR" \
...
announce_step 10 "Extract genotype-position amino-acid sequences" \
  "subtype workbook: $OUTPUT_DIR/NS5B_Subtype_AllStudies_WSeqs.xlsx; FASTA pool: $FASTA_POOL" \
  "amino-acid workbook: $OUTPUT_DIR/NS5B_Profile_Input_Source.xlsx"
...
"$PYTHON_BIN" "$SCRIPT_DIR/build_ns5b_subtype_with_gt_aa.py" \
  ...
  --output-dir "$OUTPUT_DIR" \
  --output-workbook "$OUTPUT_DIR/NS5B_Profile_Input_Source.xlsx" \
...
announce_step 12 "Export consensus FASTA files" \
  "profile workbooks: $OUTPUT_DIR/NS5B_GT_CompleteProfiles_TabsPerGT.xlsx; $OUTPUT_DIR/NS5B_Subtype_CompleteProfiles_TabsPerGT.xlsx" \
  "consensus FASTAs: $OUTPUT_DIR/NS5B_GT_Consensus.fasta; $OUTPUT_DIR/NS5B_Subtype_Consensus.fasta"
...
announce_step 13 "Build genotype RAS profile" \
  "genotype profile workbook: $OUTPUT_DIR/NS5B_GT_CompleteProfiles_TabsPerGT.xlsx" \
  "RAS profile: $OUTPUT_DIR/NS5B_GT_RAS_Profiles.xlsx"
...
announce_step 14 "Build subtype RAS profile" \
  "subtype profile workbook: $OUTPUT_DIR/NS5B_Subtype_CompleteProfiles_TabsPerGT.xlsx" \
  "RAS profile: $OUTPUT_DIR/NS5B_Subtype_RAS_Profiles.xlsx"
...
announce_step 15 "Combine genotype and subtype RAS profiles" \
  "GT RAS profile: $OUTPUT_DIR/NS5B_GT_RAS_Profiles.xlsx; subtype RAS profile: $OUTPUT_DIR/NS5B_Subtype_RAS_Profiles.xlsx" \
  "combined RAS profile: $OUTPUT_DIR/NS5B_Combined_RAS_Profiles.xlsx"
...
"$PYTHON_BIN" "$SCRIPT_DIR/build_ns5b_combined_ras_profiles.py" \
  --gt-ras-profile-workbook "$OUTPUT_DIR/NS5B_GT_RAS_Profiles.xlsx" \
...
"$PYTHON_BIN" "$REPO_ROOT/build_comet_workflow_sequence_audit/build_comet_workflow_sequence_audit.py" \
  ...
  --profile-accessions-csv "$OUTPUT_DIR/NS5B_Profile_Accessions.csv" --combined-profile-workbook "$OUTPUT_DIR/NS5B_Combined_RAS_Profiles.xlsx" \
  --profile-filter-reason "Missing or non-callable amino acid at one or more required RAS positions (150, 159, 206, 282, 316, 320, 321)" \
  --output-xlsx "$OUTPUT_DIR/NS5B_Workflow_Sequence_Audit.xlsx"
...
announce_step 16 "Build genotype amino-acid distance matrix" \
  ...
  "distance workbook: $OUTPUT_DIR/NS5B_GT_AA_Distance_Pos150_321.xlsx"
...
announce_step 17a "Build paired AA and NA distance matrices" \
  "profile accession set: $OUTPUT_DIR/NS5B_Profile_Accessions.csv" \
  "paired-distance workbooks under: $OUTPUT_DIR"
for type in aa na; do
  upper_type="$(printf '%s' "$type" | tr '[:lower:]' '[:upper:]')"
  "$PYTHON_BIN" "$SCRIPT_DIR/build_ns5b_${type}_distance_matrices.py" --input-workbook "$AA_TMP_WORKBOOK" --profile-accessions-csv "$OUTPUT_DIR/NS5B_Profile_Accessions.csv" --sequence-type "$type" --positions "150,159,206,282,316,320,321" --gt-output-xlsx "$OUTPUT_DIR/NS5B_GT_${upper_type}_Distance_RAS.xlsx" --subtype-output-xlsx "$OUTPUT_DIR/NS5B_Subtype_${upper_type}_Distance_RAS.xlsx" --min-subtype-sequences 10
  "$PYTHON_BIN" "$SCRIPT_DIR/build_ns5b_${type}_distance_matrices.py" --input-workbook "$AA_TMP_WORKBOOK" --profile-accessions-csv "$OUTPUT_DIR/NS5B_Profile_Accessions.csv" --sequence-type "$type" --positions "$(seq 150 321 | paste -sd, -)" --gt-output-xlsx "$OUTPUT_DIR/NS5B_GT_${upper_type}_Distance_Pos150_321.xlsx" --subtype-output-xlsx "$OUTPUT_DIR/NS5B_Subtype_${upper_type}_Distance_Pos150_321.xlsx" --min-subtype-sequences 10
done

announce_step 18 "Build RAS entropy reports" \
  "profile workbooks: $OUTPUT_DIR/NS5B_GT_CompleteProfiles_TabsPerGT.xlsx; $OUTPUT_DIR/NS5B_Subtype_CompleteProfiles_TabsPerGT.xlsx" \
  "entropy workbooks: $OUTPUT_DIR/NS5B_GT_RAS_Entropy.xlsx; $OUTPUT_DIR/NS5B_Subtype_RAS_Entropy.xlsx"
