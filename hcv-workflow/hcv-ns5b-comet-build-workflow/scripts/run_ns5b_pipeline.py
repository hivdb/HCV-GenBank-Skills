...
class Pipeline:
    ...
    def ensure_summary_directories(self) -> None:
        ...
        for name in (
            ...
            "build_ns5b_subtype_aa_distance_matrices",
        ):
    ...
    def steps(self) -> list[Step]:
        ...
        gt_workbook = self.output_dir / "NS5B_GT_AllStudies.xlsx"
        subtype_workbook = self.output_dir / "NS5B_Subtype_AllStudies_WSeqs.xlsx"
        gt_profile = self.output_dir / "NS5B_GT_CompleteProfiles_TabsPerGT.xlsx"
        subtype_profile = self.output_dir / "NS5B_Subtype_CompleteProfiles_TabsPerGT.xlsx"
        gt_consensus = self.output_dir / "NS5B_GT_Consensus.fasta"
        subtype_consensus = self.output_dir / "NS5B_Subtype_Consensus.fasta"
        gt_ras = self.output_dir / "NS5B_GT_RAS_Profiles.xlsx"
        subtype_ras = self.output_dir / "NS5B_Subtype_RAS_Profiles.xlsx"
        combined_ras = self.output_dir / "NS5B_Combined_RAS_Profiles.xlsx"
        annotated_combined_ras = self.output_dir / "NS5B_Combined_RAS_Profiles_Annotated.xlsx"
        ...
        def extract_aa() -> None:
            self.run("build_ns5b_subtype_with_gt_aa.py", "--subtype-workbook", subtype_workbook, "--fasta-dir", self.fasta_pool, "--gt-aa-json", self.gt_aa_json, "--output-dir", self.output_dir, "--output-workbook", self.output_dir / "NS5B_Profile_Input_Source.xlsx", stdout_path=summary("build_ns5b_subtype_with_gt_aa"), stream_output=True)

        def report_profile_counts() -> None:
            output = self.run("build_ns5b_completeprofiles_tabspergt.py", "--input-workbook", self.aa_workbook, "--report-only", stdout_path=self.temp_root / "profile_input_counts.json")
            print(output, end="")
        ...
        return [
            ...
            Step("build-genotype-workbook", "build the COMET genotype workbook with non-COMET priority assignments", lambda: self.run("build_ns5b_comet_gt_allstudies.py", "--fasta-dir", self.included_fasta_dir, "--comet-genotype-csv", comet_gt_csv, "--noncomet-coverage-csv", self.noncomet_coverage_csv, "--output-dir", self.output_dir, stdout_path=summary("build_ns5b_gt_allstudies"))),
            Step("add-genotype-counts", "add the genotype-count worksheet", lambda: self.run("add_gt_counts_sheet.py", "--workbook", gt_workbook)),
            Step("build-subtype-workbook", "build COMET subtype workbook with non-COMET priorities", lambda: self.run("build_ns5b_comet_subtype_allstudies.py", "--genotype-workbook", gt_workbook, "--comet-subtype-csv", comet_subtype_csv, "--noncomet-coverage-csv", self.noncomet_coverage_csv, "--output-dir", self.output_dir, stdout_path=summary("build_ns5b_subtype_allstudies_wseqs"))),
            ...
            Step("summarize-qc-mutation-burden", "summarize QC-passed genotype mutation burden", lambda: self.run("build_qc_passed_genotype_mutation_burden_summary.py", "--input-workbook", self.aa_workbook, "--output-csv", self.output_dir / "NS5B_QC_Passed_Genotype_Mutation_Burden_Summary.csv")),
            Step("report-profile-input-counts", "report profile-input inclusion counts", report_profile_counts),
            Step("build-complete-profiles", "build genotype and subtype complete profiles", lambda: self.run("build_ns5b_completeprofiles_tabspergt.py", "--input-workbook", self.aa_workbook, "--output-dir", self.output_dir, "--profile-accessions-csv", self.profile_accessions_csv, stdout_path=summary("build_ns5b_completeprofiles_tabspergt"))),
            Step("export-noncomet-priority-accessions", "export non-COMET priority profile accessions", lambda: self.run("export_noncomet_priority_profile_accessions.py", "--profile-accessions-csv", self.profile_accessions_csv, "--comet-subtype-csv", self.comet_csv, "--noncomet-coverage-csv", self.noncomet_coverage_csv, "--output-csv", self.output_dir / "NS5B_NonComet_Priority_Profile_Accessions.csv")),
