...
STEP_NAMES = (
    ...
    "build-combined-ras-reports", "add-nonconsensus-row", "update-coverage-labels",
...
class Pipeline:
    ...
    def steps(self) -> list[Step]:
        ...
        def prepare_comet() -> None:
            ...
            self.run("prepare_comet_ns5b_assignments.py", "--comet-csv", self.comet_csv, "--fasta-dir", self.included_fasta_dir, "--accessions-csv", self.kept_accessions_csv, "--genotype-output-csv", comet_gt_csv, "--subtype-output-csv", comet_subtype_csv, "--not-found-output-csv", self.temp_root / "comet_ns5b_not_found_or_unassigned.csv", "--not-found-fasta-output", self.temp_root / "comet_ns5b_not_found_or_unassigned.fasta", "--remove-unassigned")
        ...
        return [
            ...
            Step("build-subtype-ras-profile", "build subtype RAS profile", lambda: self.run("build_ns5b_subtype_ras_profiles.py", "--subtype-profile-workbook", subtype_profile, "--gt-aa-json", self.gt_aa_json, "--output-dir", self.step_dir("build-subtype-ras-profile"), stdout_path=summary("build-subtype-ras-profile"))),
            Step("build-combined-ras-reports", "build combined RAS, coverage, and sequence-audit reports", lambda: (self.run("build_ns5b_combined_ras_profiles.py", "--gt-ras-profile-workbook", gt_ras, "--subtype-ras-profile-workbook", subtype_ras, "--output-xlsx", combined_ras, stdout_path=summary("build-combined-ras-reports")), self.run("build_comet_subtype_ras_coverage_report.py", "--gene", "NS5B", "--combined-profile-workbook", combined_ras, "--profile-input-workbook", self.aa_workbook, "--profile-accessions-csv", self.profile_accessions_csv, "--output-xlsx", self.step_dir("build-combined-ras-reports") / "NS5B_Subtype_RAS_Coverage_Report.xlsx"), self.run("build_comet_workflow_sequence_audit.py", "--gene", "NS5B", "--selection-workbook", self.excel_file, "--selection-sheet", self.sheet_name, "--fasta-dir", self.fasta_pool, "--metadata-csv", self.metadata_csv, "--comet-csv", self.comet_csv, "--qc-workbook", self.aa_workbook, "--profile-accessions-csv", self.profile_accessions_csv, "--combined-profile-workbook", combined_ras, "--profile-filter-reason", "Missing or non-callable amino acid at one or more required RAS positions (150, 159, 206, 282, 316, 320, 321)", "--output-xlsx", self.step_dir("build-combined-ras-reports") / "NS5B_Workflow_Sequence_Audit.xlsx"))),
