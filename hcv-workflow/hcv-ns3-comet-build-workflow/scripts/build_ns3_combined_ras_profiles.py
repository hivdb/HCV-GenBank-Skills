#!/usr/bin/env python3
"""Combine NS3 genotype and subtype RAS profile workbooks by genotype."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Combine NS3 GT and subtype RAS profiles, retaining subtype amino-acid "
    ...
    parser.add_argument("--output-xlsx", default="outputs/NS3_Combined_RAS_Profiles.xlsx")
...
def main() -> int:
    ...
    full_profile_subtype_at_least_10_count = sum(
        (count := total_sequences(row[0])) is not None and count >= MIN_TOTAL_SEQUENCES
        for row in subtype_rows
    )
    print(
        json.dumps(
