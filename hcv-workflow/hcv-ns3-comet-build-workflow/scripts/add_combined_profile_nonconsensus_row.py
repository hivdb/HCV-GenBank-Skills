...
def fasta(path: Path) -> dict[str, str]:
    result: dict[str, str] = {}; header = None; parts: list[str] = []
    for raw in path.read_text().splitlines():
        line = raw.strip()
        if line.startswith(">"):
            if header: result[header] = "".join(parts)
...
def main() -> None:
    ...
    source_wb = load_workbook(a.profile_input_workbook, read_only=True, data_only=True); source = source_wb.active
    headers = [str(c.value or "") for c in next(source.iter_rows(min_row=1, max_row=1))]; ix = {v: i for i, v in enumerate(headers)}
    for row in source.iter_rows(min_row=2, values_only=True):
        accession = str(row[ix["AccessionID"]] or "").strip(); gt = str(row[ix["ClosestGT"]] or "").strip().removeprefix("GT"); subtype = str(row[ix["ClosestSubtype"]] or "").strip().lower()
        if (accession, gt, subtype) not in allowed or (gt, subtype) not in included: continue
        if "AlignmentQCStatus" in ix and str(row[ix["AlignmentQCStatus"]] or "").strip() != "PASS": continue
        start, sequence = row[ix["StartAAPosition"]], str(row[ix["AASequence"]] or "").strip().upper()
        if not start or gt not in references: continue
        for pos in positions:
            offset = pos - int(start)
            if not 0 <= offset < len(sequence): continue
            aa = sequence[offset]
            if aa not in NORMAL_AAS: continue
            denominator[pos] += 1
