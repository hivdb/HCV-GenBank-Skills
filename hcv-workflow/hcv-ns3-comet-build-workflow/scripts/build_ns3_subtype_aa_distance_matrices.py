...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-xlsx", default="outputs/NS3_Subtype_AA_Distance_Pos36_175.xlsx"
    )
    parser.add_argument(
        "--temp-dir", default="outputs/comet-NS3/temp/ns3_subtype_aa_distance_matrices"
    )
    ...
    try:
        for sheet_name in wb.sheetnames:
            ...
            for row in ws.iter_rows(min_row=2, values_only=True):
                ...
                value = row[index["NumSeqsIncludingPosition"]]
                if not subtype or value in (None, ""):
                    continue
                label = f"{sheet_name.strip()}_{subtype}"
                counts[label] = max(counts.get(label, 0), int(value))
    finally:
        wb.close()
    return counts
