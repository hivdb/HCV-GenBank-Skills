...
REFERENCE_AA_LENGTH = 631


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Export NS3 consensus amino-acid FASTA records from GT and subtype complete-profile workbooks."
    )
...
def script_temp_dir() -> Path:
    path = Path(os.environ.get("NS3_STEP_OUTPUT_DIR", "outputs/comet-NS3-one-ras/temp")) / Path(__file__).stem
...
def load_gt_entries(workbook_path: Path) -> tuple[list[tuple[str, str]], dict[str, list[int]]]:
    ...
    for sheet_name in wb.sheetnames:
        ...
        # Preserve fixed NS3 coordinates; missing calls must not shift later AAs.
...
def load_subtype_entries(workbook_path: Path) -> tuple[list[tuple[str, str]], dict[str, list[int]]]:
    ...
    for sheet_name in wb.sheetnames:
        ...
        for subtype in sorted(best_by_subtype_position):
            ...
            if not positions:
                continue
            # Preserve fixed NS3 coordinates; X denotes an unavailable position.
...
def main() -> int:
    ...
    gt_fasta = Path(args.gt_output_fasta).expanduser() if args.gt_output_fasta else output_dir / "NS3_GT_Consensus.fasta"
    subtype_fasta = (
        Path(args.subtype_output_fasta).expanduser()
        if args.subtype_output_fasta
        else output_dir / "NS3_Subtype_Consensus.fasta"
    )
