...
def complete_profile_exclusion_reasons(
    output_dir: Path,
    candidate_accessions: dict[str, str],
) -> dict[str, str]:
    """Describe why candidate accessions were not retained by profile construction."""
    rule = ""
    ...
    if summary_path.is_file():
        try:
            ...
            pass
    reason = (
        f"Excluded because it did not meet the complete-profile input rule: {rule}"
        if rule
        else "Excluded because it was not included in complete-profile construction"
    )
...
def main() -> int:
    ...
    snapshots: dict[str, tuple[dict[str, str] | None, dict[str, str]]] = {
        ...
        "build-complete-profiles": (
            profile_rows,
            complete_profile_exclusion_reasons(
                step("build-complete-profiles"),
                qc_rows,
            ),
        ),
    }
