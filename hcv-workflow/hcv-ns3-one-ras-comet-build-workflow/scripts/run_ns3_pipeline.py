...
SKILL_NAME = "hcv-ns3-one-ras-comet-build-workflow"
...
def load_config(config_path: Path) -> dict[str, str]:
    ...
    if config_path.is_file():
        ...
        for section_name in ("common", "ns3_comet", "ns3_one_ras"):
...
class Pipeline:
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        self.output_dir = path_value(
            value("output_dir", "OUTPUT_DIR", "outputs/comet-NS3-one-ras")
        )
        ...
        self.comet_subtype_assignments_csv = (
            self.temp_root / "comet_ns3_subtype_assignments.csv"
        )
        self.priority_assignments_csv = (
            self.step_dir("select-noncomet-priority-assignments")
            / "NS3_NonComet_Priority_Assignments.csv"
        )
        self.aa_workbook = (
            self.step_dir("validate-profile-alignment")
            / "NS3_Profile_Input_Alignment_QC.xlsx"
        )
    ...
    def print_gt7_gt8_subtype_counts(self) -> None:
        ...
        if (
            ...
            extraction_workbook = (
                self.step_dir("extract-profile-aa") / "NS3_Profile_Input_Source.xlsx"
            )
    ...
    def steps(self) -> list[Step]:
        ...
        gt_workbook = (
            self.step_dir("build-genotype-workbook") / "NS3_GT_AllStudies.xlsx"
        )
        subtype_workbook = (
            self.step_dir("build-subtype-workbook")
            / "NS3_Subtype_AllStudies_WSeqs.xlsx"
        ...
        gt_consensus = (
            self.step_dir("export-consensus-fastas") / "NS3_GT_Consensus.fasta"
        )
        subtype_consensus = (
            self.step_dir("export-consensus-fastas") / "NS3_Subtype_Consensus.fasta"
        )
        gt_ras = (
            self.step_dir("build-genotype-ras-profile") / "NS3_GT_RAS_Profiles.xlsx"
        )
        subtype_ras = (
            self.step_dir("build-subtype-ras-profile") / "NS3_Subtype_RAS_Profiles.xlsx"
        )
        explicit_subtype_ras = (
            self.step_dir("build-subtype-ras-profile")
            / "NS3_Subtype_RAS_Profiles_Explicit_AA.xlsx"
        )
        combined_ras = (
        ...
        annotated_combined_ras = (
            ...
            / "NS3_Combined_RAS_Profiles_Annotated.xlsx"
        )
        ...
        def discover() -> None:
            output = self.run(
                ...
                "--output-dir",
                self.discovery_tmp,
                stdout_path=self.discovery_tmp / "discovery_ns3.json",
                stream_output=True,
            )
            del output
            matches = sorted(
        ...
        def prepare_comet() -> None:
            ...
            self.run(
                "prepare_comet_ns3_assignments.py",
                "--comet-csv",
                self.comet_csv,
                "--fasta-dir",
                self.included_fasta_dir,
        ...
        def extract_aa() -> None:
            ...
            self.run(
                "build_ns3_subtype_with_gt_aa.py",
                "--subtype-workbook",
                subtype_workbook,
                "--fasta-dir",
                self.fasta_pool,
                ...
                step_dir,
                "--output-workbook",
                step_dir / "NS3_Profile_Input_Source.xlsx",
        ...
        def report_profile_counts() -> None:
            output = self.run(
                "build_ns3_completeprofiles_tabspergt.py",
                "--input-workbook",
                self.aa_workbook,
                "--report-only",
            ...
            distribution = summary.get("included_genotype_distribution", {})
            print(
                f"Complete-profile genotype distribution: {format_genotype_distribution(distribution)}"
            )
        ...
        def paired_distances() -> None:
            for sequence_type in ("aa", "na"):
                ...
                for suffix, range_arguments in (
                    ("RAS", ()),
                    ("Pos36_175", ("--start", "36", "--end", "175")),
                ):
                    ...
                    self.run(
                        ...
                        self.profile_accessions_csv,
                        "--gt-output-xlsx",
                        step_dir / f"NS3_GT_{upper}_Distance_{suffix}.xlsx",
                        "--subtype-output-xlsx",
        ...
        return [
            Step(
                "prepare-workdirs",
                "recreate the NS3 COMET output and run directories",
                prepare,
            ),
            Step(
            ...
            Step(
                ...
                lambda: (
                    self.run(
                        "prepare_ns3_pipeline_workdirs.py",
                        "--clean-dir",
                        self.refid_metadata_dir,
                    ),
                    self.run(
            ...
            Step(
                ...
                lambda: self.run(
                    "build_ns3_comet_gt_allstudies.py",
                    "--fasta-dir",
                    self.filtered_fasta_dir,
                    "--comet-genotype-csv",
                    comet_gt_csv,
            ...
            Step(
                ...
                lambda: self.run(
                    "build_ns3_comet_subtype_allstudies.py",
                    "--genotype-workbook",
                    gt_workbook,
                    "--comet-subtype-csv",
                    comet_subtype_csv,
            ...
            Step(
                ...
                lambda: self.run(
                    ...
                    self.step_dir("validate-profile-alignment")
                    / "NS3_Profile_Alignment_QC_Flagged_Accessions.csv",
            ...
            Step(
                ...
                lambda: self.run(
                    ...
                    / "NS3_QC_Passed_Genotype_Mutation_Burden_Summary.csv",
                ),
            ),
            Step(
                "report-profile-input-counts",
            ...
            Step(
                "export-accession-aa-wide-table",
                "export accession amino-acid calls at positions 36 through 175",
                lambda: self.run(
                    ...
                    / "NS3_Profile_Accession_AA_Calls_Wide_Pos36_175.csv",
                    "--copy-output-csv",
                    REPO_ROOT
                    / "covariant_analysis/NS3_Profile_Accession_AA_Calls_Wide_Pos36_175.csv",
            ...
            Step(
                ...
                lambda: self.run(
                    ...
                    / "NS3_NonComet_Priority_Profile_Accessions.csv",
                ),
            ),
            Step(
                "export-consensus-fastas",
            ...
            Step(
                ...
                lambda: self.run(
                    "align_ns3_subtype_consensuses_to_gt1a.py",
                    "--input-fasta",
                    subtype_consensus,
                    "--output-fasta",
                    self.step_dir("align-subtype-consensus-to-gt1a")
                    / "NS3_Subtype_Consensus_Aligned_to_GT1_1a.fasta",
                ),
            ),
            Step(
                "compare-reference-consensus",
                ...
                lambda: (
                    self.run(
                        ...
                        REPO_ROOT
                        / "HCVData/Subtype-Ref/HCV_Subtype_Refs_NS3_AA_Pairwise_Aligned.fasta",
                        "--subtype-consensus-fasta",
                        self.step_dir("align-subtype-consensus-to-gt1a")
                        / "NS3_Subtype_Consensus_Aligned_to_GT1_1a.fasta",
                        "--subtype-length-csv",
                        self.step_dir("compare-reference-consensus")
                        / "NS3_Subtype_Reference_Consensus_AA_Lengths.csv",
                    ),
                    self.run(
                        "build_ns3_subtype_consensus_reference_distance.py",
                        "--subtype-profile-workbook",
                        subtype_profile,
                        "--subtype-json",
                        self.subtype_json,
                        "--output-xlsx",
                        self.step_dir("compare-reference-consensus")
                        / "NS3_Subtype_Consensus_Reference_AA_Distance_RAS.xlsx",
                    ),
                ),
            ),
            Step(
            ...
            Step(
                ...
                lambda: (
                    self.run(
                        "build_ns3_subtype_ras_profiles.py",
                        "--subtype-profile-workbook",
                        subtype_profile,
            ...
            Step(
                ...
                lambda: (
                    self.run(
                        "build_ns3_combined_ras_profiles.py",
                        "--gt-ras-profile-workbook",
                        gt_ras,
                    ...
                    self.run(
                        ...
                        "--output-xlsx",
                        self.step_dir("build-combined-ras-reports")
                        / "NS3_Subtype_RAS_Coverage_Report.xlsx",
                    ),
                    self.run(
            ...
            Step(
                ...
                lambda: self.run(
                    ...
                    / "NS3_Subtype_RAS_Consensus_Difference_Summary.xlsx",
                    "--output-png",
                    self.step_dir("summarize-subtype-ras-differences")
                    / "NS3_Subtype_RAS_Consensus_Difference_Trend.png",
            ...
            Step(
                ...
                lambda: self.run(
                    "build_ns3_gt_aa_distance_matrix.py",
                    "--input-fasta",
                    gt_consensus,
                    "--aligned-fasta",
                    self.step_dir("build-genotype-aa-consensus-distance")
                    / "NS3_GT_Consensus_aligned.fasta",
                    "--output-xlsx",
                    self.step_dir("build-genotype-aa-consensus-distance")
                    / "NS3_GT_Consensus_AA_Distance_Pos36_175.xlsx",
                    "--details-xlsx",
                    summary(
                        "build-genotype-aa-consensus-distance",
                        "NS3_GT_Consensus_AA_Distance_Pos36_175_details.xlsx",
                    ),
                    "--start",
                    "36",
                    "--end",
            ...
            Step(
                ...
                lambda: self.run(
                    "build_ns3_subtype_aa_distance_matrices.py",
                    "--input-fasta",
                    subtype_consensus,
                    "--subtype-profile-workbook",
                    subtype_profile,
                    ...
                    / "NS3_Subtype_Consensus_AA_Distance_Pos36_175.xlsx",
                    "--temp-dir",
                    self.step_dir("build-subtype-aa-consensus-distance"),
                    "--start",
                    "36",
            ...
            Step(
                ...
                lambda: self.run(
                    "build_ns3_ras_entropy.py",
                    "--gt-profile-workbook",
                    gt_profile,
                    "--subtype-profile-workbook",
                    subtype_profile,
                    "--gt-output-xlsx",
                    self.step_dir("build-ras-entropy") / "NS3_GT_RAS_Entropy.xlsx",
                    "--subtype-output-xlsx",
                    self.step_dir("build-ras-entropy") / "NS3_Subtype_RAS_Entropy.xlsx",
            ...
            Step(
                ...
                lambda: self.run(
                    "analyze_ns3_genotype_subtype_aa_predictability.py",
                    "--subtype-profile-workbook",
                    merged_subtype_profile,
                    "--min-subtype-sequences",
                    "1",
                    ...
                    / "NS3_Genotype_Subtype_AA_Predictability.xlsx",
                    stdout_path=summary(
                        "analyze-genotype-subtype-aa-predictability",
            ...
            Step(
                ...
                lambda: self.run(
                    ...
                    / "NS3_GT7_GT8_Step_Sequence_Audit.csv",
                    "--accessions-csv",
                    self.step_dir("audit-gt7-gt8-sequences")
                    / "NS3_GT7_GT8_Step_Sequence_Audit_Accessions.csv",
                    "--summary-xlsx",
                    self.step_dir("audit-gt7-gt8-sequences")
                    / "NS3_GT7_GT8_Step_Sequence_Audit_Summary.xlsx",
                    "--summary-markdown",
                    self.step_dir("audit-gt7-gt8-sequences")
                    / "NS3_GT7_GT8_Step_Sequence_Audit_Summary.md",
                ),
            ),
            Step(
                "compare-gt7-gt8-local-assignments",
                "compare GT7 and GT8 workflow subtype calls with local NS3 assignments",
                lambda: self.run(
                    "compare_ns3_gt7_gt8_local_assignments.py",
                    "--gene",
                    "NS3",
                    "--subtype-workbook",
                    subtype_workbook,
                    ...
                    / "NS3_GT7_GT8_Local_Assignment_Comparison.xlsx",
                    "--output-csv",
                    self.step_dir("compare-gt7-gt8-local-assignments")
                    / "NS3_GT7_GT8_Local_Assignment_Comparison.csv",
                    stdout_path=summary("compare-gt7-gt8-local-assignments"),
                ),
            ),
            Step(
                "collect-report",
                "copy final NS3 report artifacts into the report folder",
                lambda: self.run(
