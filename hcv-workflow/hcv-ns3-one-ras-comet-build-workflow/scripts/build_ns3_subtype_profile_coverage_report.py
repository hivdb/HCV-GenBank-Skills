...
DEFAULT_RAS_POSITIONS = (36, 41, 43, 54, 55, 56, 80, 122, 155, 156, 158, 166, 168, 170, 175)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--ras-positions", default=",".join(map(str, DEFAULT_RAS_POSITIONS)))
...
def load_source_rows(path: Path, accessions: set[str], genotype: str, subtype: str) -> dict[str, list[dict[str, object]]]:
    ...
    for values in worksheet.iter_rows(min_row=2, values_only=True):
        ...
        if accession not in accessions or row_genotype != genotype or row_subtype != subtype.lower():
            continue
        rows_by_accession[accession].append({name: values[position] for name, position in index.items()})
    workbook.close()
...
def coverage_row(
    ...
    for row in source_rows:
        ...
        range_valid = sum(calls.get(position) in VALID_AAS for position in range_positions)
        candidates.append((ras_valid, range_valid, len(calls), row, calls))
    if not candidates:
        return {
            "Accession": accession, "SourceRecordCount": 0, "CoverageStatus": "MISSING_FROM_QC_WORKBOOK",
            "RASPositionsCovered": 0, "RangePositionsCovered": 0,
            "_calls": {},
        }
    ...
    covered_ras = [position for position in ras_positions if calls.get(position) in VALID_AAS]
    missing_ras = [position for position in ras_positions if position not in covered_ras]
    start = row.get("StartAAPosition")
    ...
    return {
        ...
        "CoverageStatus": "COMPLETE_RAS_AND_RANGE" if ras_valid == len(ras_positions) and range_valid == len(range_positions)
        else "COMPLETE_RAS" if ras_valid == len(ras_positions) else "PARTIAL_RAS",
        ...
        **{f"P{position}": calls.get(position, "") if calls.get(position) in VALID_AAS else "" for position in ras_positions},
        "_calls": calls,
    }


def write_workbook(path: Path, rows: list[dict[str, object]], genotype: str, subtype: str, range_positions: tuple[int, ...], ras_positions: tuple[int, ...]) -> None:
    workbook = Workbook()
    ...
    summary.append(["Complete RAS coverage", sum(row.get("CompleteRASCoverage") == "Yes" for row in rows)])
    summary.append(["Complete range coverage", sum(row.get("CompleteRangeCoverage") == "Yes" for row in rows)])
    summary.append(["Missing from QC workbook", sum(row.get("CoverageStatus") == "MISSING_FROM_QC_WORKBOOK" for row in rows)])
    summary.append([])
    summary.append(["Coverage status", "Accession count"])
    for status, count in sorted(Counter(str(row.get("CoverageStatus", "")) for row in rows).items()):
        summary.append([status, count])

    detail = workbook.create_sheet("Accession_Coverage")
    columns = [
        "Accession", "SourceRecordCount", "SelectedRefID", "SelectedRefName", "StartAAPosition", "EndAAPosition",
        "AlignmentQCStatus", "CoverageStatus", "RASPositionsCovered", "RASPositionsMissing", "RASCovered", "RASMissing",
        "CompleteRASCoverage", "RangePositionsCovered", "RangePositionCount", "RangeCoveragePercent", "CompleteRangeCoverage",
        *[f"P{position}" for position in ras_positions],
    ]
    detail.append(columns)
    for row in sorted(rows, key=lambda item: (str(item.get("CoverageStatus")) != "COMPLETE_RAS_AND_RANGE", str(item["Accession"]))):
    ...
    for column, width in {"A": 16, "B": 18, "C": 14, "D": 28, "E": 17, "F": 15, "G": 20, "H": 27, "I": 19, "J": 19, "K": 34, "L": 34, "M": 21, "N": 22, "O": 18, "P": 22, "Q": 23}.items():
...
def write_position_coverage_csv(
    path: Path, rows: list[dict[str, object]], range_positions: tuple[int, ...], ras_positions: tuple[int, ...]
) -> None:
    """Write raw and unambiguous accession coverage at every requested position."""
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("w", encoding="utf-8", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=["Pos", "IsRASPosition", "#Seq", "#NonMixture"])
        writer.writeheader()
...
def write_position_coverage_png(
    path: Path, rows: list[dict[str, object]], range_positions: tuple[int, ...], ras_positions: tuple[int, ...], genotype: str, subtype: str
) -> None:
    """Plot raw positional coverage, retaining ambiguous and stop AA calls."""
    coverage = [sum(bool(dict(row.get("_calls", {})).get(position, "")) for row in rows) for position in range_positions]
    colors = ["#C0504D" if position in ras_positions else "#5B9BD5" for position in range_positions]
    figure, axis = plt.subplots(figsize=(max(22, len(range_positions) * 0.08), 7), constrained_layout=True)
    axis.bar(range_positions, coverage, color=colors, width=0.9, linewidth=0)
    axis.set_title(f"NS3 GT{genotype}_{subtype}: sequence coverage by amino-acid position")
...
def main() -> int:
    ...
    ras_positions = tuple(int(value) for value in args.ras_positions.split(",") if value.strip())
    range_positions = tuple(range(args.range_start, args.range_end + 1))
    accessions = load_profile_accessions(Path(args.profile_accessions_csv), args.genotype, args.subtype)
    source_rows = load_source_rows(Path(args.profile_input_workbook), accessions, args.genotype, args.subtype)
    rows = [coverage_row(accession, source_rows.get(accession, []), range_positions, ras_positions) for accession in accessions]
    write_workbook(Path(args.output_xlsx), rows, args.genotype, args.subtype, range_positions, ras_positions)
    write_position_coverage_csv(Path(args.position_coverage_csv), rows, range_positions, ras_positions)
    write_position_coverage_png(Path(args.position_coverage_png), rows, range_positions, ras_positions, args.genotype, args.subtype)
    print(f"Wrote {len(rows)} accession rows to {Path(args.output_xlsx).resolve()}")
    print(f"Wrote position coverage CSV to {Path(args.position_coverage_csv).resolve()}")
    print(f"Wrote position coverage chart to {Path(args.position_coverage_png).resolve()}")
    return 0
