---
name: hcv-ns3-one-ras-comet-build-workflow
...
```bash
python hcv-workflow/hcv-ns3-one-ras-comet-build-workflow/scripts/run_ns3_pipeline.py --list-steps
python hcv-workflow/hcv-ns3-one-ras-comet-build-workflow/scripts/run_ns3_pipeline.py
python hcv-workflow/hcv-ns3-one-ras-comet-build-workflow/scripts/run_ns3_pipeline.py --step discover-refid-fastas
```
...
The variant inherits settings from `[common]` and `[ns3_comet]`. Add `[ns3_one_ras]` to `pipeline.local.toml` to set its isolated `output_dir` and, when needed, a different `sheet_name`.
Each stage writes its outputs under a numbered directory in `outputs/comet-NS3-one-ras/`.
...
- `HCVData/nonComet-Full-genome/NS3_AllSeq_NonComet_Coverage.csv` for mandatory non-COMET subtype 1d and genotype 7/8 overrides and additions
...
Complete-profile construction additionally requires a callable standard amino acid at one or more NS3 RAS positions: 36, 41, 43, 54, 55, 56, 80, 122, 155, 156, 158, 166, 168, 170, or 175. Missing, `X`, stop (`*`), or non-standard calls at every one of these positions exclude the accession from the profile and downstream combined RAS reports.
...
- discovery `filtered_rows.xlsx` under `outputs/comet-NS3-one-ras/temp/.../find_refid_fastas/...`
...
- Keep temporary outputs under `outputs/comet-NS3-one-ras/temp/` so they do not mix with other skills.
