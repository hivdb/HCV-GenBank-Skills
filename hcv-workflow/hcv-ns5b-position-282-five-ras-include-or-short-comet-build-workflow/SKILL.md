---
name: hcv-ns5b-position-282-five-ras-include-or-short-comet-build-workflow
description: Use this skill for HCV NS5B position-282-plus-five-RAS COMET builds using the Include-or-Short reference-selection workbook.
---

# HCV NS5B Position-282 Plus Five-RAS Include-or-Short COMET Build Workflow
...
```bash
python hcv-workflow/hcv-ns5b-position-282-five-ras-include-or-short-comet-build-workflow/scripts/run_ns5b_pipeline.py --list-steps
python hcv-workflow/hcv-ns5b-position-282-five-ras-include-or-short-comet-build-workflow/scripts/run_ns5b_pipeline.py
python hcv-workflow/hcv-ns5b-position-282-five-ras-include-or-short-comet-build-workflow/scripts/run_ns5b_pipeline.py --step discover-refid-fastas
```
...
The variant inherits settings from `[common]` and `[ns5b_comet]`. Its `[ns5b_position_282_five_ras_include_or_short]` configuration selects `HCVData/Ref-selection/IncludedNS5BRefs_StatusIncludeOrShort.xlsx` and its isolated output directory.
Each stage writes its outputs under a numbered directory in `outputs/comet-NS5B-position-282-five-ras-include-or-short/`.
...
Complete-profile construction retains an accession only when it has callable standard amino acids at NS5B position 282 and at least five of the other RAS positions: 150, 159, 206, 316, 320, and 321. A missing, `X`, stop (`*`), or non-standard call at position 282—or fewer than five callable calls among those other RAS positions—excludes that accession from the profile and its downstream RAS and distance reports. The remaining RAS positions are still included in downstream RAS reports.
## Outputs
...
The workflow writes NS5B outputs under `outputs/comet-NS5B-position-282-five-ras-include-or-short/`, including:

- `NS5B_GT_AllStudies.xlsx`
- `NS5B_matched_fasta_files.txt`
- discovery `filtered_rows.xlsx` under `outputs/comet-NS5B-position-282-five-ras-include-or-short/temp/.../find_refid_fastas/...`
...
- Keep temporary outputs under `outputs/comet-NS5B-position-282-five-ras-include-or-short/temp/` so they do not mix with other skills.
