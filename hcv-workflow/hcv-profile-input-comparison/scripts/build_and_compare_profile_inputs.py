...
def profile_rows(source: Path) -> dict[str, tuple[str, str]]:
    ...
    if missing := required.difference(idx):
        raise RuntimeError(
            f"Columns missing from {source}: {', '.join(sorted(missing))}"
        )
    ...
    for values in ws.iter_rows(min_row=2, values_only=True):
        ...
        if (
            not accession
            or not values[idx["AASequence"]]
            or values[idx["StartAAPosition"]] in (None, "")
            or values[idx["EndAAPosition"]] in (None, "")
        ):
            continue
...
def read_csv(path: Path) -> dict[str, tuple[str, str]]:
    with path.open(encoding="utf-8-sig", newline="") as handle:
        return {
            r["accession"].strip(): (r["genotype"].strip(), r["subtype"].strip())
...
def write_comparison(
    ...
    summary = [
        ...
        ("shared_accession_count", len(shared)),
        ("same_genotype_count", same_gt),
        ("different_genotype_count", diff_gt),
        (
            "different_genotype_percent",
            f"{100 * diff_gt / len(shared) if shared else 0:.2f}",
        ),
        ("same_subtype_count", same_subtype),
        ("different_subtype_count", diff_subtype),
        (
            "different_subtype_percent",
            f"{100 * diff_subtype / len(shared) if shared else 0:.2f}",
        ),
    ]
    with (out / f"{gene}_Profile_Input_Assignment_Comparison.csv").open(
        "w", encoding="utf-8", newline=""
    ) as handle:
        writer = csv.writer(handle)
        writer.writerow(["metric", "value"])
        writer.writerows(summary)
    with (out / f"{gene}_Profile_Input_Assignment_Differences.csv").open(
        "w", encoding="utf-8", newline=""
    ) as handle:
        writer = csv.writer(handle)
        writer.writerow(
    ...
    for metric, value in summary[4:]:
        print(f"{gene.lower()}_{metric}={value}")
...
def main() -> int:
    ...
    for gene in GENES:
        ...
        if not source.is_file():
            print(f"{gene.lower()}_status=comet_profile_input_source_missing")
            continue
        ...
        write_comparison(
            ...
            local,
            comparison_dir,
            "ok" if local_path.is_file() else "local_profile_input_csv_missing",
        )
