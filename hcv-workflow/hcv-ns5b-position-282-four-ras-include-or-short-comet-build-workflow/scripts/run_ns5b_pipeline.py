...
STEP_NAMES = (
    "prepare-workdirs", "discover-refid-fastas", "stage-refid-fastas", "prepare-comet-assignments", "select-noncomet-priority-assignments",
    "filter-accession-metadata", "split-refid-metadata", "filter-refid-fastas", "build-genotype-workbook",
    "add-genotype-counts", "build-subtype-workbook", "extract-profile-aa", "validate-profile-alignment",
    "summarize-qc-mutation-burden", "report-profile-input-counts", "build-complete-profiles", "merge-subtype-complete-profiles",
    ...
    "publish-ictv-report", "audit-gt7-gt8-sequences", "compare-gt7-gt8-local-assignments",
)
...
def load_config(config_path: Path) -> dict[str, str]:
    ...
    if config_path.is_file():
        ...
        for section_name in ("common", "ns5b_comet", "ns5b_position_282_four_ras_include_or_short"):
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--workers", type=int, help="Worker threads for NS5B Step 11 accession extraction and BLASTX.")
...
class Pipeline:
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        self.output_dir = path_value(value("output_dir", "OUTPUT_DIR", "outputs/comet-NS5B-position-282-four-ras-include-or-short"))
        ...
        self.noncomet_coverage_csv = path_value(value("noncomet_coverage_csv", "NONCOMET_COVERAGE_CSV", "HCVData/HCV-all-seq-subtype/NS5B_AllSeq_NonComet_Coverage.csv"))
        self.reference_subtypes_csv = REPO_ROOT / "HCVData/Reference_seqs/HCV_Subtype_Refs_AA_Accession_Subtype.csv"
        self.workers = args.workers if args.workers is not None else int(values.get("NS5B_WORKERS", "4"))
        ...
        self.comet_subtype_assignments_csv = self.temp_root / "comet_ns5b_subtype_assignments.csv"
    ...
    def print_gt7_gt8_subtype_counts(self) -> None:
        ...
        if self.current_step_order >= STEP_ORDER["build-complete-profiles"] and profile_accessions_csv.is_file():
            ...
            extraction_workbook = self.step_dir("extract-profile-aa") / "NS5B_Profile_Input_Source.xlsx"
    ...
    def steps(self) -> list[Step]:
        ...
        gt_workbook = self.step_dir("build-genotype-workbook") / "NS5B_GT_AllStudies.xlsx"
        subtype_workbook = self.step_dir("build-subtype-workbook") / "NS5B_Subtype_AllStudies_WSeqs.xlsx"
        gt_profile = self.step_dir("build-complete-profiles") / "NS5B_GT_CompleteProfiles_TabsPerGT.xlsx"
        subtype_profile = self.step_dir("build-complete-profiles") / "NS5B_Subtype_CompleteProfiles_TabsPerGT.xlsx"
        gt_consensus = self.step_dir("export-consensus-fastas") / "NS5B_GT_Consensus.fasta"
        subtype_consensus = self.step_dir("export-consensus-fastas") / "NS5B_Subtype_Consensus.fasta"
        gt_ras = self.step_dir("build-genotype-ras-profile") / "NS5B_GT_RAS_Profiles.xlsx"
        subtype_ras = self.step_dir("build-subtype-ras-profile") / "NS5B_Subtype_RAS_Profiles.xlsx"
        explicit_subtype_ras = self.step_dir("build-subtype-ras-profile") / "NS5B_Subtype_RAS_Profiles_Explicit_AA.xlsx"
        combined_ras = self.step_dir("build-combined-ras-reports") / "NS5B_Combined_RAS_Profiles.xlsx"
        annotated_combined_ras = self.step_dir("add-nonconsensus-row") / "NS5B_Combined_RAS_Profiles_Annotated.xlsx"
        ...
        def discover() -> None:
            output = self.run("find_refid_fastas.py", "--excel-file", self.excel_file, "--sheet", self.sheet_name, "--fasta-dir", self.fasta_pool, "--output-dir", self.discovery_tmp, stdout_path=self.discovery_tmp / "discovery_ns5b.json", stream_output=True)
            del output
        ...
        def prepare_comet() -> None:
            ...
            self.run("prepare_comet_ns5b_assignments.py", "--comet-csv", self.comet_csv, "--fasta-dir", self.included_fasta_dir, "--genotype-output-csv", comet_gt_csv, "--subtype-output-csv", comet_subtype_csv, "--not-found-output-csv", self.temp_root / "comet_ns5b_not_found_or_unassigned.csv", "--not-found-fasta-output", self.temp_root / "comet_ns5b_not_found_or_unassigned.fasta", "--remove-unassigned")

        def extract_aa() -> None:
            step_dir = self.step_dir("extract-profile-aa")
            self.run("build_ns5b_subtype_with_gt_aa.py", "--subtype-workbook", subtype_workbook, "--fasta-dir", self.fasta_pool, "--gt-aa-json", self.gt_aa_json, "--workers", str(self.workers), "--output-dir", step_dir, "--output-workbook", step_dir / "NS5B_Profile_Input_Source.xlsx", stdout_path=summary("extract-profile-aa"), stream_output=True)

        def report_profile_counts() -> None:
            output = self.run("build_ns5b_completeprofiles_tabspergt.py", "--input-workbook", self.aa_workbook, "--report-only", stdout_path=self.step_dir("report-profile-input-counts") / "profile_input_counts.json")
            print(output, end="")
        ...
        def paired_distances() -> None:
            for sequence_type in ("aa", "na"):
                ...
                for positions, suffix in ((RAS_POSITIONS, "RAS"), (RANGE_POSITIONS, "Pos150_321")):
                    step_dir = self.step_dir("build-paired-distance-matrices")
                    self.run(script, "--input-workbook", self.aa_workbook, "--profile-accessions-csv", self.profile_accessions_csv, "--sequence-type", sequence_type, "--positions", positions, "--gt-output-xlsx", step_dir / f"NS5B_GT_{upper}_Distance_{suffix}.xlsx", "--subtype-output-xlsx", step_dir / f"NS5B_Subtype_{upper}_Distance_{suffix}.xlsx", "--min-subtype-sequences", "10")
        ...
        return [
            ...
            Step("build-complete-profiles", "build genotype and subtype complete profiles", lambda: self.run("build_ns5b_completeprofiles_tabspergt.py", "--input-workbook", self.aa_workbook, "--output-dir", self.step_dir("build-complete-profiles"), "--profile-accessions-csv", self.profile_accessions_csv, stdout_path=summary("build-complete-profiles"))),
            Step("merge-subtype-complete-profiles", "merge subtype complete-profile worksheets into one table", lambda: self.run("merge_ns5b_subtype_completeprofiles.py", "--input-workbook", subtype_profile, "--output-workbook", self.step_dir("merge-subtype-complete-profiles") / "NS5B_Subtype_CompleteProfiles_Merged.xlsx", stdout_path=summary("merge-subtype-complete-profiles"))),
            Step("export-noncomet-priority-accessions", "export non-COMET priority profile accessions", lambda: self.run("export_noncomet_priority_profile_accessions.py", "--profile-accessions-csv", self.profile_accessions_csv, "--comet-subtype-csv", self.comet_csv, "--noncomet-coverage-csv", self.noncomet_coverage_csv, "--output-csv", self.step_dir("export-noncomet-priority-accessions") / "NS5B_NonComet_Priority_Profile_Accessions.csv")),
            Step("export-consensus-fastas", "export genotype and subtype consensus FASTA files", lambda: self.run("export_ns5b_consensus_fasta.py", "--gt-profile-workbook", gt_profile, "--subtype-profile-workbook", subtype_profile, "--output-dir", self.step_dir("export-consensus-fastas"))),
            Step("align-subtype-consensus-to-gt1a", "align subtype consensuses to the fixed GT1_1a coordinate system", lambda: self.run("align_ns5b_subtype_consensuses_to_gt1a.py", "--input-fasta", subtype_consensus, "--output-fasta", self.step_dir("align-subtype-consensus-to-gt1a") / "NS5B_Subtype_Consensus_Aligned_to_GT1_1a.fasta")),
            Step("compare-reference-consensus", "compare GT1_1a-aligned COMET consensuses to pairwise-aligned subtype references", lambda: (self.run("export_gt_reference_consensus_differences.py", "--gene", "NS5B", "--reference-fasta", REPO_ROOT / "HCVData/HCV_GT_Refs_NS3_NS5A_NTD_NS5B_AA.fasta", "--consensus-dir", self.step_dir("export-consensus-fastas"), "--output-dir", self.step_dir("compare-reference-consensus"), "--subtype-reference-fasta", REPO_ROOT / "HCVData/Reference_seqs/HCV_Subtype_Refs_NS5B_AA_Pairwise_Aligned.fasta", "--subtype-consensus-fasta", self.step_dir("align-subtype-consensus-to-gt1a") / "NS5B_Subtype_Consensus_Aligned_to_GT1_1a.fasta", "--subtype-length-csv", self.step_dir("compare-reference-consensus") / "NS5B_Subtype_Reference_Consensus_AA_Lengths.csv"), self.run("build_ns5b_subtype_consensus_reference_distance.py", "--subtype-profile-workbook", subtype_profile, "--subtype-json", self.subtype_json, "--output-xlsx", self.step_dir("compare-reference-consensus") / "NS5B_Subtype_Consensus_Reference_AA_Distance_RAS.xlsx"))),
            Step("build-genotype-ras-profile", "build genotype RAS profile", lambda: self.run("build_ns5b_gt_ras_profiles.py", "--gt-profile-workbook", gt_profile, "--gt-aa-json", self.gt_aa_json, "--output-dir", self.step_dir("build-genotype-ras-profile"), stdout_path=summary("build-genotype-ras-profile"))),
            ...
            Step("compare-gt7-gt8-local-assignments", "compare GT7 and GT8 workflow subtype calls with local NS5B assignments", lambda: self.run("compare_ns5b_gt7_gt8_local_assignments.py", "--gene", "NS5B", "--subtype-workbook", subtype_workbook, "--local-assignments-csv", REPO_ROOT / "outputs/folder_assignments/NS5B_assignments.csv", "--output-xlsx", self.step_dir("compare-gt7-gt8-local-assignments") / "NS5B_GT7_GT8_Local_Assignment_Comparison.xlsx", "--output-csv", self.step_dir("compare-gt7-gt8-local-assignments") / "NS5B_GT7_GT8_Local_Assignment_Comparison.csv", stdout_path=summary("compare-gt7-gt8-local-assignments"))),
        ]
