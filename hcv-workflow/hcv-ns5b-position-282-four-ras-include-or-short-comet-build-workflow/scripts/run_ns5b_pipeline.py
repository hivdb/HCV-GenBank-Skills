#!/usr/bin/env python3
"""Run named stages of the HCV NS5B COMET build workflow.

Examples:
  scripts/run_ns5b_pipeline.py --list-steps
  scripts/run_ns5b_pipeline.py
  scripts/run_ns5b_pipeline.py --step discover-refid-fastas
"""
...
SKILL_NAME = "hcv-ns5b-position-282-four-ras-comet-build-workflow"
RAS_POSITIONS = "150,159,206,282,316,320,321"
RANGE_POSITIONS = ",".join(str(position) for position in range(150, 322))
STEP_NAMES = (
    "prepare-workdirs",
    "discover-refid-fastas",
    "stage-refid-fastas",
    "prepare-comet-assignments",
...
def load_config(config_path: Path) -> dict[str, str]:
    ...
    if config_path.is_file():
        ...
        for section_name in (
            "common",
            "ns5b_comet",
            "ns5b_position_282_four_ras_include_or_short",
        ):
            section = config.get(section_name, {})
            if isinstance(section, dict):
                values.update(
                    {key.upper(): str(value) for key, value in section.items()}
                )
    return values
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--list-steps", action="store_true", help="Print available step names and exit"
    )
    parser.add_argument(
        "--step",
        action="append",
        help="Run only this named step; repeat to run multiple steps",
    )
    parser.add_argument(
        "--config", type=Path, default=REPO_ROOT / "pipeline.local.toml"
    )
    ...
    parser.add_argument(
        "--workers",
        type=int,
        help="Worker threads for NS5B Step 11 accession extraction and BLASTX.",
    )
...
class Pipeline:
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        def value(argument: str, environment: str, default: str = "") -> str:
            ...
            return (
                supplied if supplied is not None else values.get(environment, default)
            )

        self.python_bin = executable_value(
            value("python_bin", "PYTHON_BIN", str(REPO_ROOT / ".venv/bin/python"))
        )
        ...
        self.output_dir = path_value(
            value(
                "output_dir",
                "OUTPUT_DIR",
                "outputs/comet-NS5B-position-282-four-ras-include-or-short",
            )
        )
        self.reference_fasta = path_value(
            value("reference_fasta", "REFERENCE_FASTA", "HCVData/HCV_GT_RefSeqs.fasta")
        )
        self.subtype_json = path_value(
            value(
                "subtype_json",
                "SUBTYPE_JSON",
                "HCVData/HCV_Subtype_Refs_By_Genome_NA.json",
            )
        )
        self.gt_aa_json = path_value(
            value("gt_aa_json", "GT_AA_JSON", "HCVData/HCV_GT_Refs_By_Gene_AA.json")
        )
        self.metadata_csv = path_value(
            value(
                "accessions_metadata_csv",
                "ACCESSIONS_METADATA_CSV",
                "HCVData/Accessions_metadata.csv",
            )
        )
        self.comet_csv = path_value(
            value(
                "comet_subtyping_csv",
                "COMET_SUBTYPING_CSV",
        ...
        self.workers = (
            ...
            else int(values.get("NS5B_WORKERS", "4"))
        )
        self.temp_root = path_value(
            value(
                "temp_root",
                "TEMP_ROOT",
                str(self.step_dir("prepare-comet-assignments")),
            )
        )
        self.discovery_tmp = self.step_dir("discover-refid-fastas")
        self.matched_txt = self.discovery_tmp / "NS5B_matched_fasta_files.txt"
        self.staged_fasta_dir = (
            self.step_dir("stage-refid-fastas") / "included_refid_fastas"
        )
        self.metadata_dir = self.step_dir("filter-accession-metadata")
        self.refid_metadata_dir = (
            self.step_dir("split-refid-metadata") / "refid_metadata"
        )
        self.filtered_fasta_dir = (
            self.step_dir("filter-refid-fastas") / "included_refid_fastas"
        )
        self.kept_accessions_csv = (
            self.step_dir("filter-refid-fastas") / "kept_accessions.csv"
        )
        self.included_fasta_dir = (
            self.step_dir("prepare-comet-assignments") / "included_refid_fastas"
        )
        self.comet_subtype_assignments_csv = (
            self.temp_root / "comet_ns5b_subtype_assignments.csv"
        )
        self.priority_assignments_csv = (
            self.step_dir("select-noncomet-priority-assignments")
            / "NS5B_NonComet_Priority_Assignments.csv"
        )
        self.aa_workbook = (
            self.step_dir("validate-profile-alignment")
            / "NS5B_Profile_Input_Alignment_QC.xlsx"
        )
        self.profile_accessions_csv = (
            self.step_dir("build-complete-profiles")
            / "NS5B_Profile_Accessions_QC_Pass.csv"
        )
    ...
    def run(
        ...
        *arguments: str,
        ...
        command = [
            self.python_bin,
            repository_relative(SCRIPT_DIR / script),
            *(repository_relative(argument) for argument in arguments),
        ]
        environment = {**os.environ, "NS5B_STEP_OUTPUT_DIR": str(self.current_step_dir)}
    ...
    def print_accession_counts(self, input_accessions: set[str]) -> None:
        ...
        if (
            self.current_step_order >= STEP_ORDER["build-complete-profiles"]
            and profile_accessions_csv.is_file()
        ):
            with profile_accessions_csv.open(encoding="utf-8", newline="") as handle:
                for row in csv.DictReader(handle):
                    accession = (
                        row.get("accession")
                        or row.get("AccessionID")
    ...
    def print_gt7_gt8_subtype_counts(self) -> None:
        ...
        if (
            ...
            stage = (
                "90%-range-coverage profile eligibility"
                if profile_accessions_csv != self.profile_accessions_csv
                else "complete-profile eligibility"
            )
            with profile_accessions_csv.open(encoding="utf-8", newline="") as handle:
                for row in csv.DictReader(handle):
                    genotype = (row.get("genotype") or "").strip().removeprefix("GT")
            ...
            self.current_step_order >= STEP_ORDER["validate-profile-alignment"]
            and self.aa_workbook.is_file()
        ):
            stage = "alignment QC"
            ...
            header = [
                str(value or "") for value in next(sheet.iter_rows(values_only=True))
            ]
            ...
            for values in sheet.iter_rows(min_row=2, values_only=True):
                if str(values[index["AlignmentQCStatus"]] or "").strip() != "PASS":
                    continue
                genotype = (
                    str(values[index["ClosestGT"]] or "").strip().removeprefix("GT")
                )
            ...
            extraction_workbook = (
                self.step_dir("extract-profile-aa") / "NS5B_Profile_Input_Source.xlsx"
            )
            if extraction_workbook.is_file():
                ...
                workbook = load_workbook(
                    extraction_workbook, read_only=True, data_only=True
                )
                sheet = workbook[workbook.sheetnames[0]]
                header = [
                    str(value or "")
            ...
            with self.comet_subtype_assignments_csv.open(
            ...
            if self.priority_assignments_csv.is_file():
                with self.priority_assignments_csv.open(
                    encoding="utf-8-sig", newline=""
                ) as handle:
                    for row in csv.DictReader(handle):
                        accession = (row.get("Accession") or "").strip()
            ...
            for accession in self.fasta_accessions(fasta_dir):
                ...
                assignment = priority_assignments.get(
                    accession_key
                ) or comet_assignments.get(accession_key)
            ...
            for accession_key, (genotype, subtype) in priority_assignments.items():
                if (
                    accession_key not in seen_accessions
                    and genotype in counts
                    and subtype
                ):
                    counts[genotype][0].add(subtype)
                    counts[genotype] = (counts[genotype][0], counts[genotype][1] + 1)
        for genotype in ("7", "8"):
            ...
            print(
                f"GT{genotype} after {stage or 'assignment'}: {len(subtypes)} subtypes ({accessions} accessions)"
            )

    def steps(self) -> list[Step]:
        summary = lambda step, filename="last_run_summary.json": (
            self.step_dir(step) / filename
        )
        gt_workbook = (
            self.step_dir("build-genotype-workbook") / "NS5B_GT_AllStudies.xlsx"
        )
        subtype_workbook = (
            self.step_dir("build-subtype-workbook")
            / "NS5B_Subtype_AllStudies_WSeqs.xlsx"
        ...
        subtype_consensus = (
            self.step_dir("export-consensus-fastas") / "NS5B_Subtype_Consensus.fasta"
        )
        gt_ras = (
            self.step_dir("build-genotype-ras-profile") / "NS5B_GT_RAS_Profiles.xlsx"
        )
        subtype_ras = (
            self.step_dir("build-subtype-ras-profile")
            / "NS5B_Subtype_RAS_Profiles.xlsx"
        )
        explicit_subtype_ras = (
            self.step_dir("build-subtype-ras-profile")
            / "NS5B_Subtype_RAS_Profiles_Explicit_AA.xlsx"
        )
        combined_ras = (
            self.step_dir("build-combined-ras-reports")
            / "NS5B_Combined_RAS_Profiles.xlsx"
        )
        annotated_combined_ras = (
            self.step_dir("add-nonconsensus-row")
            / "NS5B_Combined_RAS_Profiles_Annotated.xlsx"
        )
        ...
        def build_90pct_range_coverage_combined_profile() -> None:
            ...
            gt_profile_90pct = (
                complete_profile_dir / "NS5B_GT_CompleteProfiles_TabsPerGT.xlsx"
            )
            subtype_profile_90pct = (
                complete_profile_dir / "NS5B_Subtype_CompleteProfiles_TabsPerGT.xlsx"
            )
            profile_accessions_90pct = (
                step_dir / "NS5B_Profile_Accessions_90Pct_Range_Coverage.csv"
            )
            ...
            combined_90pct = (
                step_dir / "NS5B_Combined_RAS_Profiles_90Pct_Range_Coverage.xlsx"
            )
            self.run(
                "build_ns5b_completeprofiles_tabspergt.py",
                "--input-workbook",
                self.aa_workbook,
                "--output-dir",
                complete_profile_dir,
                ...
                "--range-end",
                "321",
                stdout_path=summary("build-90pct-range-coverage-combined-profile"),
            )
            self.run(
                "build_ns5b_gt_ras_profiles.py",
                "--gt-profile-workbook",
                gt_profile_90pct,
                "--gt-aa-json",
                self.gt_aa_json,
            ...
            self.run(
                "build_ns5b_subtype_ras_profiles.py",
                "--subtype-profile-workbook",
                subtype_profile_90pct,
                "--gt-ras-profile-workbook",
                gt_ras_90pct,
                "--gt-aa-json",
                self.gt_aa_json,
                "--output-dir",
            ...
            self.run(
                "build_ns5b_combined_ras_profiles.py",
                "--gt-ras-profile-workbook",
                gt_ras_90pct,
                "--subtype-ras-profile-workbook",
                subtype_ras_90pct,
                "--output-xlsx",
                combined_90pct,
            )
            self.run(
                "replace_comet_profile_coverage_range_with_mean_diff.py",
        ...
        def prepare() -> None:
            self.run(
                "prepare_ns5b_pipeline_workdirs.py", "--clean-dir", self.output_dir
            )

        def discover() -> None:
            output = self.run(
                "find_refid_fastas.py",
                "--excel-file",
                self.excel_file,
                "--sheet",
                self.sheet_name,
                ...
                "--output-dir",
                self.discovery_tmp,
                stdout_path=self.discovery_tmp / "discovery_ns5b.json",
                stream_output=True,
            )
            del output
            matches = sorted(
                self.discovery_tmp.glob("refid_fasta_*/matched_fasta_files.txt")
            )
            if len(matches) != 1:
                raise RuntimeError(
                    f"Expected one matched FASTA file list under {self.discovery_tmp}, found {len(matches)}"
                )
            self.matched_txt.write_text(
                matches[0].read_text(encoding="utf-8"), encoding="utf-8"
            )

        def prepare_comet() -> None:
            shutil.copytree(
                self.staged_fasta_dir, self.included_fasta_dir, dirs_exist_ok=True
            )
            self.run(
                "prepare_comet_ns5b_assignments.py",
                "--comet-csv",
                self.comet_csv,
                "--fasta-dir",
                self.included_fasta_dir,
                ...
                "--subtype-output-csv",
                comet_subtype_csv,
                "--not-found-output-csv",
                self.temp_root / "comet_ns5b_not_found_or_unassigned.csv",
                "--not-found-fasta-output",
                self.temp_root / "comet_ns5b_not_found_or_unassigned.fasta",
                "--unassigned-count-output",
                self.step_dir("prepare-comet-assignments")
                / "NS5B_Comet_Unassigned_Accession_Count.txt",
                "--remove-unassigned",
            )
        ...
        def extract_aa() -> None:
            ...
            self.run(
                "build_ns5b_subtype_with_gt_aa.py",
                "--subtype-workbook",
                subtype_workbook,
                "--fasta-dir",
                self.fasta_pool,
                ...
                step_dir,
                "--output-workbook",
                step_dir / "NS5B_Profile_Input_Source.xlsx",
                stdout_path=summary("extract-profile-aa"),
                stream_output=True,
            )
        ...
        def report_profile_counts() -> None:
            output = self.run(
                ...
                self.aa_workbook,
                "--report-only",
                stdout_path=self.step_dir("report-profile-input-counts")
                / "profile_input_counts.json",
            )
            print(output, end="")
        ...
        def paired_distances() -> None:
            for sequence_type in ("aa", "na"):
                ...
                script = f"build_ns5b_{sequence_type}_distance_matrices.py"
                for positions, suffix in (
                    (RAS_POSITIONS, "RAS"),
                    (RANGE_POSITIONS, "Pos150_321"),
                ):
                    ...
                    self.run(
                        ...
                        positions,
                        "--gt-output-xlsx",
                        step_dir / f"NS5B_GT_{upper}_Distance_{suffix}.xlsx",
                        "--subtype-output-xlsx",
                        step_dir / f"NS5B_Subtype_{upper}_Distance_{suffix}.xlsx",
                        "--min-subtype-sequences",
                        "10" if sequence_type == "na" else "1",
                    )

        return [
            Step(
                "prepare-workdirs",
                "recreate the NS5B COMET output and run directories",
                prepare,
            ),
            Step(
                "discover-refid-fastas", "discover selected RefID FASTA files", discover
            ),
            Step(
                "stage-refid-fastas",
                "copy discovered FASTA files into the staging directory",
                lambda: (
                    self.staged_fasta_dir.mkdir(parents=True, exist_ok=True),
                    self.run(
                        "stage_matched_refid_fastas.py",
                        "--matched-files",
                        self.matched_txt,
                        "--output-dir",
                        self.staged_fasta_dir,
            ...
            Step(
                "prepare-comet-assignments",
                "create COMET calls and remove missing or unassigned records",
                prepare_comet,
            ),
            Step(
                "select-noncomet-priority-assignments",
                ...
                Step(
                    ...
                    lambda: (
                        self.run(
                            "prepare_ns5b_pipeline_workdirs.py",
                            "--clean-dir",
                            self.refid_metadata_dir,
                        ),
                        self.run(
                ...
                Step(
                    ...
                    lambda: self.run(
                        "build_ns5b_comet_gt_allstudies.py",
                        "--fasta-dir",
                        self.filtered_fasta_dir,
                        "--comet-genotype-csv",
                        comet_gt_csv,
                ...
                Step(
                    ...
                    lambda: self.run(
                        "build_ns5b_comet_subtype_allstudies.py",
                        "--genotype-workbook",
                        gt_workbook,
                        "--comet-subtype-csv",
                        comet_subtype_csv,
                ...
                Step(
                    ...
                    lambda: self.run(
                        "validate_ns5b_profile_alignment.py",
                        "--input-workbook",
                        self.step_dir("extract-profile-aa")
                        / "NS5B_Profile_Input_Source.xlsx",
                        "--gt-aa-json",
                        self.gt_aa_json,
                        "--output-workbook",
                        self.aa_workbook,
                        ...
                        self.step_dir("validate-profile-alignment")
                        / "NS5B_Profile_Alignment_QC_Flagged_Accessions.csv",
                ...
                Step(
                    ...
                    lambda: self.run(
                        ...
                        / "NS5B_QC_Passed_Genotype_Mutation_Burden_Summary.csv",
                    ),
                ),
                Step(
                    "report-profile-input-counts",
                ...
                Step(
                    ...
                    lambda: (
                        self.run(
                            "merge_ns5b_subtype_completeprofiles.py",
                            "--input-workbook",
                            subtype_profile,
                            "--output-workbook",
                            self.step_dir("merge-subtype-complete-profiles")
                        ...
                        self.run(
                            ...
                            "--output-csv",
                            self.step_dir("merge-subtype-complete-profiles")
                            / "NS5B_Profile_Accession_AA_Calls.csv",
                            stdout_path=summary(
                                "merge-subtype-complete-profiles",
                ...
                Step(
                    ...
                    lambda: self.run(
                        ...
                        / "NS5B_NonComet_Priority_Profile_Accessions.csv",
                    ),
                ),
                Step(
                    "export-consensus-fastas",
                    ...
                    lambda: self.run(
                        "export_ns5b_consensus_fasta.py",
                        "--gt-profile-workbook",
                        gt_profile,
                        "--subtype-profile-workbook",
                        subtype_profile,
                ...
                Step(
                    ...
                    lambda: self.run(
                        "align_ns5b_subtype_consensuses_to_gt1a.py",
                        "--input-fasta",
                        subtype_consensus,
                        "--output-fasta",
                ...
                Step(
                    ...
                    lambda: (
                        self.run(
                            ...
                            "--subtype-length-csv",
                            self.step_dir("compare-reference-consensus")
                            / "NS5B_Subtype_Reference_Consensus_AA_Lengths.csv",
                        ),
                        self.run(
                            "build_ns5b_subtype_consensus_reference_distance.py",
                            "--subtype-profile-workbook",
                            subtype_profile,
                            "--subtype-json",
                            self.subtype_json,
                            "--output-xlsx",
                            self.step_dir("compare-reference-consensus")
                            / "NS5B_Subtype_Consensus_Reference_AA_Distance_RAS.xlsx",
                        ),
                    ),
                ),
                Step(
                    ...
                    self.run(
                        ...
                        "--output-csv",
                        self.step_dir("build-genotype-ras-profile")
                        / "NS5B_GT_RAS_Ambiguity_Calls.csv",
                        stdout_path=summary(
            ...
            Step(
                ...
                lambda: (
                    self.run(
                        ...
                        "--output-dir",
                        self.step_dir("build-subtype-ras-profile"),
                        stdout_path=summary("build-subtype-ras-profile"),
                    ),
                    self.run(
                        "replace_comet_profile_coverage_range_with_mean_diff.py",
                        "--combined-profile-workbook",
                    ...
                    self.run(
                        ...
                        "--output-csv",
                        self.step_dir("build-subtype-ras-profile")
                        / "NS5B_Subtype_RAS_Ambiguity_Calls.csv",
                        stdout_path=summary(
            ...
            Step(
                "build-combined-ras-reports",
                "build combined RAS, coverage, and sequence-audit reports",
                lambda: (
                    self.run(
                        "build_ns5b_combined_ras_profiles.py",
                        "--gt-ras-profile-workbook",
                        ...
                        "--output-xlsx",
                        combined_ras,
                        stdout_path=summary("build-combined-ras-reports"),
                    ),
                    self.run(
                        "replace_comet_profile_coverage_range_with_mean_diff.py",
                        "--combined-profile-workbook",
                    ...
                    self.run(
                        "build_comet_subtype_ras_coverage_report.py",
                        "--gene",
                        "NS5B",
                        "--combined-profile-workbook",
                        combined_ras,
                        ...
                        "--output-xlsx",
                        self.step_dir("build-combined-ras-reports")
                        / "NS5B_Subtype_RAS_Coverage_Report.xlsx",
                    ),
                    self.run(
                        "build_comet_workflow_sequence_audit.py",
                        "--gene",
                        "NS5B",
                        "--selection-workbook",
                        self.excel_file,
                        ...
                        "Missing or non-callable amino acid at required NS5B position 282 or fewer than four other callable RAS positions",
                        "--output-xlsx",
                        self.step_dir("build-combined-ras-reports")
                        / "NS5B_Workflow_Sequence_Audit.xlsx",
                    ),
                ),
            ),
            Step(
                "add-nonconsensus-row",
                "create annotated combined profile with MeanDiff and PositionDiff",
                lambda: self.run(
                    "add_combined_profile_nonconsensus_row.py",
                    "--combined-profile-workbook",
                    combined_ras,
                    "--output-workbook",
                    annotated_combined_ras,
            ...
            Step(
                "summarize-subtype-ras-differences",
                "summarize subtype RAS differences from genotype consensus",
                lambda: self.run(
                    "build_ns5b_subtype_ras_consensus_difference_summary.py",
                    "--combined-profile-workbook",
                    combined_ras,
                    "--profile-input-workbook",
                    self.aa_workbook,
                    ...
                    gt_consensus,
                    "--output-xlsx",
                    self.step_dir("summarize-subtype-ras-differences")
                    / "NS5B_Subtype_RAS_Consensus_Difference_Summary.xlsx",
                    "--output-png",
                    self.step_dir("summarize-subtype-ras-differences")
                    / "NS5B_Subtype_RAS_Consensus_Difference_Trend.png",
                    stdout_path=summary("summarize-subtype-ras-differences"),
                ),
            ),
            Step(
                "build-genotype-aa-consensus-distance",
                "build genotype consensus amino-acid distance matrix",
                lambda: self.run(
                    "build_ns5b_gt_aa_distance_matrix.py",
                    "--input-fasta",
                    gt_consensus,
                    "--aligned-fasta",
                    self.step_dir("build-genotype-aa-consensus-distance")
                    / "NS5B_GT_Consensus_aligned.fasta",
                    "--output-xlsx",
                    self.step_dir("build-genotype-aa-consensus-distance")
                    / "NS5B_GT_Consensus_AA_Distance_Pos150_321.xlsx",
                    "--details-xlsx",
                    summary(
                        "build-genotype-aa-consensus-distance",
                        "NS5B_GT_Consensus_AA_Distance_Pos150_321_details.xlsx",
                    ),
                    "--start",
                    "150",
                    "--end",
                    "321",
                    stdout_path=summary(
                        "build-genotype-aa-consensus-distance", "last_run_summary.txt"
                    ),
                ),
            ),
            Step(
                "build-subtype-aa-consensus-distance",
                "build subtype consensus amino-acid distance matrices",
                lambda: self.run(
                    "build_ns5b_subtype_aa_distance_matrices.py",
                    "--input-fasta",
                    subtype_consensus,
                    "--subtype-profile-workbook",
                    subtype_profile,
                    ...
                    "10",
                    "--output-xlsx",
                    self.step_dir("build-subtype-aa-consensus-distance")
                    / "NS5B_Subtype_Consensus_AA_Distance_Pos150_321.xlsx",
                    "--temp-dir",
                    self.step_dir("build-subtype-aa-consensus-distance"),
                    "--start",
                    "150",
                    ...
                    "321",
                    stdout_path=summary(
                        "build-subtype-aa-consensus-distance", "last_run_summary.txt"
                    ),
                ),
            ),
            Step(
            ...
            Step(
                "build-ras-entropy",
                "build genotype and subtype RAS entropy reports",
                lambda: self.run(
                    "build_ns5b_ras_entropy.py",
                    "--gt-profile-workbook",
                    gt_profile,
                    "--subtype-profile-workbook",
                    subtype_profile,
                    "--gt-output-xlsx",
                    self.step_dir("build-ras-entropy") / "NS5B_GT_RAS_Entropy.xlsx",
                    "--subtype-output-xlsx",
            ...
            Step(
                "publish-ictv-report",
                "publish the NS5B ICTV comparison report",
                lambda: self.run(
                    str(
                        REPO_ROOT
                        / "hcv-workflow"
            ...
            Step(
                "audit-gt7-gt8-sequences",
                "audit GT7 and GT8 kept/excluded sequences against each previous workflow step",
                lambda: self.run(
                    "build_ns3_gt7_gt8_step_audit.py",
                    "--pipeline-output-dir",
                    self.output_dir,
                    "--gene",
                    "NS5B",
                    "--output-csv",
                    self.step_dir("audit-gt7-gt8-sequences")
                    / "NS5B_GT7_GT8_Step_Sequence_Audit.csv",
                    "--accessions-csv",
                    self.step_dir("audit-gt7-gt8-sequences")
                    / "NS5B_GT7_GT8_Step_Sequence_Audit_Accessions.csv",
                    "--summary-xlsx",
                    self.step_dir("audit-gt7-gt8-sequences")
                    / "NS5B_GT7_GT8_Step_Sequence_Audit_Summary.xlsx",
                    "--summary-markdown",
                    self.step_dir("audit-gt7-gt8-sequences")
                    / "NS5B_GT7_GT8_Step_Sequence_Audit_Summary.md",
                ),
            ),
            Step(
                "compare-gt7-gt8-local-assignments",
                "compare GT7 and GT8 workflow subtype calls with local NS5B assignments",
                lambda: self.run(
                    "compare_ns5b_gt7_gt8_local_assignments.py",
                    "--gene",
                    "NS5B",
                    "--subtype-workbook",
                    subtype_workbook,
...
def main() -> int:
    ...
    if unknown:
        raise SystemExit(
            f"Unknown step name(s): {', '.join(unknown)}. Use --list-steps."
        )
    if (
        not args.excel_file
        and not pipeline.excel_file.is_file()
        and "discover-refid-fastas" in requested
    ):
    ...
    if not pipeline.sheet_name and "discover-refid-fastas" in requested:
        raise SystemExit(
            "SHEET_NAME or --sheet-name is required for discover-refid-fastas"
        )
    ...
    for name in requested:
        ...
        input_fasta_dir = (
            ...
            if step.order <= STEP_ORDER["stage-refid-fastas"]
                ...
                pipeline.included_fasta_dir
                if step.order <= STEP_ORDER["filter-refid-fastas"]
                else pipeline.filtered_fasta_dir
            )
        )
