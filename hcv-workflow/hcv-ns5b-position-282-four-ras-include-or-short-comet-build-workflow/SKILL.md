...
Complete-profile construction retains an accession only when it has callable standard amino acids at NS5B position 282 and at least four of the other RAS positions: 150, 159, 206, 316, 320, and 321. A missing, `X`, stop (`*`), or non-standard call at position 282—or fewer than four callable calls among those other RAS positions—excludes that accession from the profile and its downstream RAS and distance reports. The remaining RAS positions are still included in downstream RAS reports.
## Outputs
...
- discovery `filtered_rows.xlsx` under `outputs/comet-NS5B-position-282-four-ras/temp/.../find_refid_fastas/...`
...
- Keep temporary outputs under `outputs/comet-NS5B-position-282-four-ras/temp/` so they do not mix with other skills.
