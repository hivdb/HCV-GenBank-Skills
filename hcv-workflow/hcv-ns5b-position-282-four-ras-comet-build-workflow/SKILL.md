---
name: hcv-ns5b-position-282-four-ras-comet-build-workflow
...
# HCV NS5B Position-282 Plus Four-RAS COMET Build Workflow
...
```bash
python hcv-workflow/hcv-ns5b-position-282-four-ras-comet-build-workflow/scripts/run_ns5b_pipeline.py --list-steps
python hcv-workflow/hcv-ns5b-position-282-four-ras-comet-build-workflow/scripts/run_ns5b_pipeline.py
python hcv-workflow/hcv-ns5b-position-282-four-ras-comet-build-workflow/scripts/run_ns5b_pipeline.py --step discover-refid-fastas
```
...
The variant inherits shared inputs from `[ns5b_comet]`. Set `sheet_name` in `[ns5b_position_282_four_ras]` of `pipeline.local.toml` only when this variant needs a different worksheet; set `output_dir` there only to override its default output folder. This variant ignores generic and `[ns5b_comet]` output settings.
Each stage writes its outputs under a numbered directory in `outputs/comet-NS5B-position-282-four-ras/`.
