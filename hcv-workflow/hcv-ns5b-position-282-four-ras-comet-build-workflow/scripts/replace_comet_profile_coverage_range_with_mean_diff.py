...
LABEL_RE = re.compile(
    r"^GT(?P<gt>\d+)(?:_(?P<subtype>\S+))? \((?P<count>\d+), [^)]+\)$"
)
...
def main() -> int:
    ...
    with Path(args.profile_accessions_csv).open(
    ...
    positions = [
        int(str(cell.value)[1:])
        for cell in worksheet[1]
        if re.fullmatch(r"P\d+", str(cell.value or ""))
    ]

    profile = load_workbook(input_path, read_only=True, data_only=True)
    source = profile.active
    headers = [
        str(cell.value or "") for cell in next(source.iter_rows(min_row=1, max_row=1))
    ]
    ...
    for row in worksheet.iter_rows(min_row=2, max_col=1):
        ...
        if not match:
            continue
        genotype, subtype, count = (
            match.group("gt"),
            match.group("subtype"),
            int(match.group("count")),
        )
        fraction = (
            non_x_calls[(genotype, subtype)] / (len(positions) * count)
            if count
            else 0.0
        )
        cell.value = (
            f"GT{genotype}{'_' + subtype if subtype else ''} ({count}, {fraction:.1%})"
        )
        changed += 1
