...
if [[ -f "$CONFIG_PATH" ]]; then
  eval "$(python3 "$SCRIPT_DIR/load_pipeline_defaults.py" ns5a_comet "$CONFIG_PATH" "$REPO_ROOT")"
fi
...
usage() {
  cat <<'EOF'
Usage:
  EXCEL_FILE=/path/to/HCV_BlastHits.xlsx SHEET_NAME=NS5A_PtGT0_Check FASTA_POOL=/path/to/FASTA [GENBANK_DIR=/path/to/genbank_seq_files] hcv-workflow/hcv-ns5a-comet-build-workflow/scripts/run_ns5a_pipeline.sh
...
COMET_SUBTYPING_CSV="${COMET_SUBTYPING_CSV:-$REPO_ROOT/HCVData/Comet Subtyping/NS5A.csv}"
NONCOMET_SUBTYPE_WORKBOOK="${NONCOMET_SUBTYPE_WORKBOOK:-$REPO_ROOT/outputs/local_alignment/NS5A_Subtype_AllStudies_WSeqs.xlsx}"
...
DISCOVERY_JSON="$DISCOVERY_TMP/discovery_ns5a.json"
SKILL_TEMP_ROOT="$REPO_ROOT/outputs/temp/$SKILL_NAME"
GT_ALLSTUDIES_JSON="$SKILL_TEMP_ROOT/build_ns5a_gt_allstudies/last_run_summary.json"
SOURCEFEATURES_JSON="$SKILL_TEMP_ROOT/build_ns5a_sourcefeatures_csv/last_run_summary.json"
SOURCEFEATURES_GROUPED_JSON="$SKILL_TEMP_ROOT/build_ns5a_sourcefeatures_grouped_csv/last_run_summary.json"
SUBTYPE_ALLSTUDIES_JSON="$SKILL_TEMP_ROOT/build_ns5a_subtype_allstudies_wseqs/last_run_summary.json"
SUBTYPE_WITH_GT_AA_JSON="$SKILL_TEMP_ROOT/build_ns5a_subtype_with_gt_aa/last_run_summary.json"
COMPLETEPROFILES_JSON="$SKILL_TEMP_ROOT/build_ns5a_completeprofiles_tabspergt/last_run_summary.json"
GT_RAS_JSON="$SKILL_TEMP_ROOT/build_ns5a_gt_ras_profiles/last_run_summary.json"
SUBTYPE_RAS_JSON="$SKILL_TEMP_ROOT/build_ns5a_subtype_ras_profiles/last_run_summary.json"
COMBINED_RAS_JSON="$SKILL_TEMP_ROOT/build_ns5a_combined_ras_profiles/last_run_summary.json"
RAS_CONSENSUS_DIFFERENCE_JSON="$SKILL_TEMP_ROOT/build_ns5a_subtype_ras_consensus_difference_summary/last_run_summary.json"
GT_AA_DISTANCE_SUMMARY="$SKILL_TEMP_ROOT/build_ns5a_gt_aa_distance_matrix/last_run_summary.txt"
SUBTYPE_AA_DISTANCE_SUMMARY="$SKILL_TEMP_ROOT/build_ns5a_subtype_aa_distance_matrices/last_run_summary.txt"
RAS_ENTROPY_SUMMARY="$SKILL_TEMP_ROOT/build_ns5a_ras_entropy/last_run_summary.txt"
REFID_METADATA_DIR="$TEMP_ROOT/refid_metadata"
COMET_GENOTYPE_CSV="$TEMP_ROOT/comet_ns5a_genotype_assignments.csv"
COMET_SUBTYPE_CSV="$TEMP_ROOT/comet_ns5a_subtype_assignments.csv"
COMET_NOT_FOUND_CSV="$TEMP_ROOT/comet_ns5a_not_found_or_unassigned.csv"
COMET_NOT_FOUND_FASTA="$TEMP_ROOT/comet_ns5a_not_found_or_unassigned.fasta"
mkdir -p "$TEMP_ROOT"
...
rm -f "$SKILL_TEMP_ROOT/build_ns5a_sourcefeatures_csv/NS5A_SourceFeatures.csv"
rm -f "$SKILL_TEMP_ROOT/build_ns5a_sourcefeatures_grouped_csv/NS5A_SourceFeatures_Grouped.csv"
...
"$PYTHON_BIN" "$SCRIPT_DIR/prepare_comet_ns5a_assignments.py" \
...
announce_step 8 "Build genotype study workbook from Comet" \
  ...
  "genotype workbook: $OUTPUT_DIR/NS5A_GT_AllStudies.xlsx"
echo "Action: write the Comet genotype for each retained accession; no genotype alignment is run."
"$PYTHON_BIN" "$SCRIPT_DIR/build_ns5a_comet_gt_allstudies.py" \
  --fasta-dir "$INCLUDED_FASTA_DIR" \
...
announce_step 8a "Add genotype-count summary" \
  "genotype workbook: $OUTPUT_DIR/NS5A_GT_AllStudies.xlsx" \
  "updated workbook: $OUTPUT_DIR/NS5A_GT_AllStudies.xlsx (Genotype_Counts sheet)"
"$PYTHON_BIN" "$SCRIPT_DIR/add_gt_counts_sheet.py" --workbook "$OUTPUT_DIR/NS5A_GT_AllStudies.xlsx"

echo "Skipping disabled NS5A source-feature extraction and grouped-summary steps"
...
announce_step 9 "Build subtype study workbook from Comet" \
  "genotype workbook: $OUTPUT_DIR/NS5A_GT_AllStudies.xlsx; Comet subtype assignments: $COMET_SUBTYPING_CSV; non-COMET 1d and genotype 7/8 subtype assignments: $NONCOMET_SUBTYPE_WORKBOOK" \
...
"$PYTHON_BIN" "$SCRIPT_DIR/build_ns5a_comet_subtype_allstudies.py" \
  --genotype-workbook "$OUTPUT_DIR/NS5A_GT_AllStudies.xlsx" \
...
announce_step 10 "Extract genotype-position amino-acid sequences" \
  "subtype workbook: $OUTPUT_DIR/NS5A_Subtype_AllStudies_WSeqs.xlsx; FASTA pool: $FASTA_POOL" \
  "amino-acid workbook: $OUTPUT_DIR/NS5A_Profile_Input_Source.xlsx"
...
"$PYTHON_BIN" "$SCRIPT_DIR/build_ns5a_subtype_with_gt_aa.py" \
  ...
  --output-dir "$OUTPUT_DIR" \
  --output-workbook "$OUTPUT_DIR/NS5A_Profile_Input_Source.xlsx" \
...
"$PYTHON_BIN" "$REPO_ROOT/export_noncomet_priority_profile_accessions/export_noncomet_priority_profile_accessions.py" \
  ...
  --noncomet-subtype-workbook "$NONCOMET_SUBTYPE_WORKBOOK" \
  --output-csv "$OUTPUT_DIR/NS5A_NonComet_Priority_Profile_Accessions.csv"

announce_step 12 "Export consensus FASTA files" \
  "profile workbooks: $OUTPUT_DIR/NS5A_GT_CompleteProfiles_TabsPerGT.xlsx; $OUTPUT_DIR/NS5A_Subtype_CompleteProfiles_TabsPerGT.xlsx" \
  "consensus FASTAs: $OUTPUT_DIR/NS5A_GT_Consensus.fasta; $OUTPUT_DIR/NS5A_Subtype_Consensus.fasta"
...
announce_step 13 "Build genotype RAS profile" \
  ...
  "RAS profile: $OUTPUT_DIR/NS5A_GT_RAS_Profiles.xlsx"
...
announce_step 14 "Build subtype RAS profile" \
  "subtype profile workbook: $OUTPUT_DIR/NS5A_Subtype_CompleteProfiles_TabsPerGT.xlsx" \
  "RAS profile: $OUTPUT_DIR/NS5A_Subtype_RAS_Profiles.xlsx"
...
announce_step 15 "Combine genotype and subtype RAS profiles" \
  "GT RAS profile: $OUTPUT_DIR/NS5A_GT_RAS_Profiles.xlsx; subtype RAS profile: $OUTPUT_DIR/NS5A_Subtype_RAS_Profiles.xlsx" \
  "combined RAS profile: $OUTPUT_DIR/NS5A_Combined_RAS_Profiles.xlsx"
...
"$PYTHON_BIN" "$SCRIPT_DIR/build_ns5a_combined_ras_profiles.py" \
  --gt-ras-profile-workbook "$OUTPUT_DIR/NS5A_GT_RAS_Profiles.xlsx" \
...
"$PYTHON_BIN" "$SCRIPT_DIR/build_comet_workflow_sequence_audit.py" \
  ...
  --combined-subtype-cutoff 5 --combined-subtype-cutoff-exclusive \
  --output-xlsx "$OUTPUT_DIR/NS5A_Workflow_Sequence_Audit.xlsx"

announce_step 15a "Summarize subtype RAS differences from genotype consensus" \
  "combined RAS profile: $OUTPUT_DIR/NS5A_Combined_RAS_Profiles.xlsx; profile sequences: $AA_TMP_WORKBOOK" \
  "difference workbook: $OUTPUT_DIR/NS5A_Subtype_RAS_Consensus_Difference_Summary.xlsx; trend chart: $OUTPUT_DIR/NS5A_Subtype_RAS_Consensus_Difference_Trend.png"
"$PYTHON_BIN" "$SCRIPT_DIR/build_ns5a_subtype_ras_consensus_difference_summary.py" \
  --combined-profile-workbook "$OUTPUT_DIR/NS5A_Combined_RAS_Profiles.xlsx" \
  --profile-input-workbook "$AA_TMP_WORKBOOK" \
  --profile-accessions-csv "$OUTPUT_DIR/NS5A_Profile_Accessions.csv" \
  --genotype-consensus-fasta "$OUTPUT_DIR/NS5A_GT_Consensus.fasta" \
  --output-xlsx "$OUTPUT_DIR/NS5A_Subtype_RAS_Consensus_Difference_Summary.xlsx" \
  --output-png "$OUTPUT_DIR/NS5A_Subtype_RAS_Consensus_Difference_Trend.png" \
  > "$RAS_CONSENSUS_DIFFERENCE_JSON"

announce_step 16 "Build genotype amino-acid distance matrix" \
  "consensus FASTA: $OUTPUT_DIR/NS5A_GT_Consensus.fasta" \
  "distance workbook: $OUTPUT_DIR/NS5A_GT_AA_Distance_Pos24_93.xlsx"
"$PYTHON_BIN" "$SCRIPT_DIR/build_ns5a_gt_aa_distance_matrix.py" \
  --input-fasta "$OUTPUT_DIR/NS5A_GT_Consensus.fasta" \
  --aligned-fasta "$OUTPUT_DIR/NS5A_GT_Consensus_aligned.fasta" \
  --output-xlsx "$OUTPUT_DIR/NS5A_GT_AA_Distance_Pos24_93.xlsx" \
  --details-xlsx "$SKILL_TEMP_ROOT/build_ns5a_gt_aa_distance_matrix/NS5A_GT_AA_Distance_Pos24_93_details.xlsx" \
  --start 24 \
  --end 93 \
  > "$GT_AA_DISTANCE_SUMMARY"

announce_step 17 "Build subtype amino-acid distance matrices" \
  "consensus FASTA: $OUTPUT_DIR/NS5A_Subtype_Consensus.fasta; minimum subtype sequences: 10" \
  "distance workbook: $OUTPUT_DIR/NS5A_Subtype_AA_Distance_Pos24_93.xlsx"
"$PYTHON_BIN" "$SCRIPT_DIR/build_ns5a_subtype_aa_distance_matrices.py" \
  --input-fasta "$OUTPUT_DIR/NS5A_Subtype_Consensus.fasta" \
  --subtype-profile-workbook "$OUTPUT_DIR/NS5A_Subtype_CompleteProfiles_TabsPerGT.xlsx" \
  --min-subtype-sequences 10 \
  --output-xlsx "$OUTPUT_DIR/NS5A_Subtype_AA_Distance_Pos24_93.xlsx" \
  --temp-dir "$SKILL_TEMP_ROOT/build_ns5a_subtype_aa_distance_matrices" \
  --start 24 \
  --end 93 \
...
announce_step 18 "Build RAS entropy reports" \
  "profile workbooks: $OUTPUT_DIR/NS5A_GT_CompleteProfiles_TabsPerGT.xlsx; $OUTPUT_DIR/NS5A_Subtype_CompleteProfiles_TabsPerGT.xlsx" \
  "entropy workbooks: $OUTPUT_DIR/NS5A_GT_RAS_Entropy.xlsx; $OUTPUT_DIR/NS5A_Subtype_RAS_Entropy.xlsx"
"$PYTHON_BIN" "$SCRIPT_DIR/build_ns5a_ras_entropy.py" \
  --gt-profile-workbook "$OUTPUT_DIR/NS5A_GT_CompleteProfiles_TabsPerGT.xlsx" \
  --subtype-profile-workbook "$OUTPUT_DIR/NS5A_Subtype_CompleteProfiles_TabsPerGT.xlsx" \
  --gt-output-xlsx "$OUTPUT_DIR/NS5A_GT_RAS_Entropy.xlsx" \
  --subtype-output-xlsx "$OUTPUT_DIR/NS5A_Subtype_RAS_Entropy.xlsx" \
  > "$RAS_ENTROPY_SUMMARY"

announce_step 19 "Add combined-profile genotype-consensus difference row" \
  "combined RAS profile: $OUTPUT_DIR/NS5A_Combined_RAS_Profiles.xlsx; QC-passed profile sequences: $AA_TMP_WORKBOOK" \
  "updated combined RAS profile: $OUTPUT_DIR/NS5A_Combined_RAS_Profiles.xlsx"
