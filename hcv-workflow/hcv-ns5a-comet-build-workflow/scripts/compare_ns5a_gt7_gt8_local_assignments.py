#!/usr/bin/env python3
"""Compare NS5A workflow GT7/GT8 subtype calls with local NS5A assignments."""
...
def read_workflow_rows(path: Path) -> list[dict[str, str]]:
    ...
    for values in worksheet.iter_rows(min_row=2, values_only=True):
        row = {column: str(value or "").strip() for column, value in zip(header, values)}
...
def read_profile_accessions(path: Path) -> set[str]:
    with path.open(encoding="utf-8-sig", newline="") as handle:
        reader = csv.DictReader(handle)
        if not reader.fieldnames or "accession" not in reader.fieldnames: raise RuntimeError(f"{path} is missing the accession column")
        return {accession_key(row.get("accession")) for row in reader if accession_key(row.get("accession"))}
...
def compare_rows(workflow_rows: list[dict[str, str]], local_assignments: dict[str, list[dict[str, str]]], reference_accessions: set[str], all_comet_subtypes: dict[str, dict[str, str]], profile_accessions: set[str]) -> list[dict[str, str]]:
    ...
    for workflow in workflow_rows:
        ...
        if not local_rows:
            output.update(LocalAssignmentStatus="NOT_FOUND", GenotypeMatchesLocal="NOT_FOUND", SubtypeMatchesLocal="NOT_FOUND")
        elif len(local_rows) > 1:
            output.update(LocalAssignmentStatus="MULTIPLE_LOCAL_ROWS", GenotypeMatchesLocal="NOT_COMPARED", SubtypeMatchesLocal="NOT_COMPARED")
        else:
            local = local_rows[0]
            local_subtype = local["subtype"]
            output.update(
                LocalAssignmentStatus="FOUND" if local_subtype else "FOUND_WITHOUT_SUBTYPE",
                ...
                GenotypeMatchesLocal="YES" if normalized_genotype(workflow["ClosestGT"]) == normalized_genotype(local["genotype"]) else "NO",
                SubtypeMatchesLocal=("YES" if normalized_subtype(workflow["ClosestSubtype"]) == normalized_subtype(local_subtype) else "NO") if local_subtype else "NO_LOCAL_SUBTYPE",
            )
        rows.append(output)
    return rows
...
def write_report(rows: list[dict[str, str]], output_xlsx: Path, output_csv: Path) -> None:
    ...
    for field in ("LocalAssignmentStatus", "GenotypeMatchesLocal", "SubtypeMatchesLocal"):
...
def main() -> int:
    ...
    if profile_accessions_csv is None or not profile_accessions_csv.is_file(): raise SystemExit(f"Profile RAS-coverage accessions CSV was not found for {subtype_workbook}")
