...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--local-assignments-csv")
...
class Pipeline:
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        self.local_assignments_csv = path_value(value("local_assignments_csv", "LOCAL_ASSIGNMENTS_CSV", "outputs/folder_assignments/NS5A_assignments.csv"))
    ...
    def steps(self) -> list[Step]:
        ...
        return [
            ...
            Step("select-noncomet-priority-assignments", "select local assignments that override or supplement COMET", lambda: self.run("select_noncomet_priority_assignments.py", "--local-assignments-csv", self.local_assignments_csv, "--fasta-dir", self.fasta_pool, "--comet-unassigned-csv", self.temp_root / "comet_ns5a_not_found_or_unassigned.csv", "--output-csv", self.priority_assignments_csv, stdout_path=summary("select-noncomet-priority-assignments"))),
            ...
            Step("export-noncomet-priority-accessions", "export local-priority profile accessions", lambda: self.run("export_noncomet_priority_profile_accessions.py", "--profile-accessions-csv", self.profile_accessions_csv, "--comet-subtype-csv", self.comet_csv, "--local-assignments-csv", self.local_assignments_csv, "--output-csv", self.step_dir("export-noncomet-priority-accessions") / "NS5A_NonComet_Priority_Profile_Accessions.csv")),
