...
class Pipeline:
    ...
    def steps(self) -> list[Step]:
        ...
        return [
            ...
            Step("build-genotype-workbook", "build the COMET genotype workbook with non-COMET priority assignments", lambda: self.run("build_ns5a_comet_gt_allstudies.py", "--fasta-dir", self.included_fasta_dir, "--comet-genotype-csv", comet_gt_csv, "--noncomet-subtype-workbook", self.noncomet_workbook, "--output-dir", self.output_dir, stdout_path=summary("build_ns5a_gt_allstudies"))),
            ...
            Step("compare-reference-consensus", "compare GT1_1a-aligned COMET consensuses to pairwise-aligned subtype references", lambda: (self.run("export_gt_reference_consensus_differences.py", "--gene", "NS5A_NTD", "--reference-fasta", REPO_ROOT / "HCVData/HCV_GT_Refs_NS3_NS5A_NTD_NS5B_AA.fasta", "--consensus-dir", self.output_dir, "--subtype-reference-fasta", REPO_ROOT / "HCVData/Reference_seqs/HCV_Subtype_Refs_NS5A_NTD_AA_FirstSeq_Pairwise_Aligned.fasta", "--subtype-consensus-fasta", self.output_dir / "NS5A_Subtype_Consensus_Aligned_to_GT1_1a.fasta", "--subtype-length-csv", REPO_ROOT / "outputs/reference_seqs/NS5A_Subtype_Reference_Consensus_AA_Lengths.csv"), self.run("build_ns5a_subtype_consensus_reference_distance.py", "--subtype-profile-workbook", subtype_profile, "--subtype-json", self.subtype_json, "--output-xlsx", self.output_dir / "NS5A_Subtype_Consensus_Reference_AA_Distance_RAS.xlsx"))),
