...
def main() -> int:
    a = parse_args()
    combined = load_workbook(a.combined_profile_workbook, read_only=True, data_only=True)
    combined_sheet = combined.active
    positions = [int(str(c.value)[1:]) for c in combined_sheet[1] if re.fullmatch(r"P\d+", str(c.value or ""))]
    combined_subtypes = {
    ...
    for row in ws.iter_rows(min_row=2, values_only=True):
        ...
        if (gt, subtype) not in combined_subtypes:
            continue
        if "AlignmentQCStatus" in ix and str(row[ix["AlignmentQCStatus"]] or "").strip() != "PASS":
            continue
        start, sequence = row[ix["StartAAPosition"]], str(row[ix["AASequence"]] or "").strip().upper()
        if start and sequence:
            groups[(gt, subtype)].append((int(start), sequence))
    wb.close()

    out = Workbook(); summary = out.active; summary.title = "Subtype_Summary"
    summary.append(["Gene", "Genotype", "Subtype", "ProfileSequences", "RASPositionCount", "FullyCoveredPositions", "PartiallyCoveredPositions", "Conclusion"])
    detail = out.create_sheet("Position_Coverage")
    detail.append(["Gene", "Genotype", "Subtype", "Position", "ProfileSequences", "NonXCalls", "NoCallOrX", "CoveragePercent", "CoverageStatus"])
    header_fill = PatternFill(fill_type="solid", fgColor="D9EAF7")
    for sheet in (summary, detail):
        for cell in sheet[1]: cell.font = Font(bold=True); cell.fill = header_fill
        sheet.freeze_panes = "A2"
    for (gt, subtype), sequences in sorted(groups.items(), key=lambda x: (int(x[0][0]), x[0][1])):
        total = combined_subtypes[(gt, subtype)]; covered, partial, critically_low = [], [], []
        for position in positions:
            non_x = sum(0 <= position - start < len(sequence) and sequence[position - start] != "X" for start, sequence in sequences)
            ...
            detail.append([a.gene, f"GT{gt}", subtype, position, total, non_x, total - non_x, pct, status])
        if not partial:
            conclusion = ""
        else:
            conclusion = f"Poorly covered (<{a.threshold:g}%): P{ranges(partial)}."
            if critically_low:
                low_text = ", ".join(f"P{position} ({count}/{total}; {pct:.1f}%)" for position, count, pct in critically_low)
                conclusion += f" Below 50%: {low_text}."
        summary.append([a.gene, f"GT{gt}", subtype, total, len(positions), f"P{ranges(covered)}", f"P{ranges(partial)}", conclusion])
    for row in detail.iter_rows(min_row=2, min_col=8, max_col=8): row[0].number_format = "0.0"
    for sheet in (summary, detail):
        for col in sheet.columns:
            sheet.column_dimensions[col[0].column_letter].width = min(max(max(len(str(c.value or "")) for c in col) + 2, 12), 80)
    Path(a.output_xlsx).parent.mkdir(parents=True, exist_ok=True); out.save(a.output_xlsx)
