...
The COMET subtype step gives priority to non-COMET subtype assignments for retained accessions called `1d` and for genotype 8 accessions; genotype 7 keeps the COMET subtype call. It also adds priority non-COMET accessions absent from COMET. Amino-acid extraction reads the configured FASTA pool for these selected rows so added accessions are available.
